...
- The persisted state-file key names (`offset`, `buffer`, `turn_count`, `pending_agent_turns`, `pending_task_notifications`) are kept in snake_case for continuity with the previous Python implementation, so an in-place upgrade keeps the existing offset instead of re-emitting the whole session.
