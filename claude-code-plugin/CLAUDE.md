...
- Per-session state (`~/.claude/state/lmnr_state.json`) stores the transcript byte offset; only advance it AFTER a successful export, so failed exports retry next Stop.
