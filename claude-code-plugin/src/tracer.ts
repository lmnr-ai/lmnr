...
function toOtelAttributes(attrs: Record<string, Json>): Attributes {
  ...
  for (const [key, value] of Object.entries(attrs)) {
    ...
    // Objects and other types are unsupported as attribute values (Python's
    // _attr_value returns None for these) — drop them.
  }
  return out;
}
...
/**
 * Owns the OTel provider + a collecting processor and mints spans. Equivalent
 * to the Python OtlpSpanCollector; `spans` holds the finished ReadableSpans.
 */
export class TraceEmitter {
  ...
  constructor(config: LaminarConfig) {
    ...
    this.tracer = provider.getTracer("lmnr.claude_code");
  }
...
/**
 * Export capped by a hard timeout so a hung connection can't stall Claude Code.
