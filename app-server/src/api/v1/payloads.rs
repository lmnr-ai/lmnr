...
use crate::{
    db::project_api_keys::ProjectApiKey, routes::types::ResponseResult, storage::StorageService,
};
...
pub async fn get_payload_response(
    ...
    let mut stream = match storage.get_stream(project_id, &bucket, &key).await {
        Ok(stream) => stream,
        Err(e) => {
