...
impl From<CHSpan> for CHSpanV2 {
    fn from(value: CHSpan) -> Self {
        Self {
            ...
            input_cost: value.input_cost,
            output_cost: value.output_cost,
            ...
            tags_array: value.tags_array,
            events: value.events,
        }
    }
}

impl From<CHSpanV2> for CHSpan {
    fn from(value: CHSpanV2) -> Self {
        Self {
            span_id: value.span_id,
            name: value.name,
            span_type: value.span_type,
...
impl ClickhouseInsertable for CHSpanV2 {
    const TABLE: Table = Table::SpansV2;

    fn to_data_plane_batch(items: Vec<Self>) -> DataPlaneBatch {
        DataPlaneBatch::Spans(items.into_iter().map(|s| s.into()).collect())
    }
}
