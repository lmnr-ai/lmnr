...
use crate::{
    ch::{ClickhouseService, logs::CHLog},
...
impl MessageHandler for LogsHandler {
    ...
    async fn handle(&self, messages: Self::Message) -> Result<(), HandlerError> {
        process_logs_batch(messages, self.ch_service.clone()).await
    }
}

#[instrument(skip(messages, ch_service))]
async fn process_logs_batch(
    ...
    if let Err(e) = ch_service.insert_batch(project_id, &ch_logs).await {
        log::error!(
