...
impl SpanBuilder {
    ...
    pub fn default(scope: &SpanScope, name: &str) -> InternalSpan {
        let span = match name {
            "generate_extraction_regex.cached" => {
                info_span!(target: INTERNAL_TRACING_TARGET, "generate_extraction_regex.cached")
            }
            _ => info_span!(target: INTERNAL_TRACING_TARGET, "user_task.default"),
        };
        Self::base(InternalSpan::wrap(span, SpanType::Default), scope).parent(scope.parent)
    }
}

/// Emit a finished `Default` span recording that the regex was served from cache rather than
/// generated by an LLM. No model/provider (so the trace UI doesn't render it as a real call); the
/// `.cached` span name is the marker.
pub(crate) fn emit_cache_hit(
    scope: &SpanScope,
    input: &serde_json::Value,
    output: &serde_json::Value,
) {
    let span = SpanBuilder::default(scope, "generate_extraction_regex.cached")
        .input(input)
        .build();
