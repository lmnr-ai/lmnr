...
pub struct InputExtractionMessage {
    ...
    /// The candidate's winner stats at enqueue time; the consumer drops
    /// the message when the current lock carries a strictly stronger
    /// published winner (a later batch superseded this candidate).
    #[serde(default)]
