...
pub async fn process_span_messages(
    ...
    // On `llm_messages` insert failure we return transient; nothing to undo
    // in Redis since `mark_seen` only runs after a successful insert (and
    // the producer-side Redis stamp is best-effort, mirrored by RMT merge).
