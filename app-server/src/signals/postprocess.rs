...
pub async fn process_event_notifications_and_clustering(
    ...
    let email_targets =
        db::alert_targets::get_email_targets_for_event(&db.pool, project_id, &event_name).await?;
    ...
    for target in &email_targets {
        ...
        if serialized_size >= mq_max_payload() {
            log::warn!(
                "MQ payload limit exceeded for email {}: payload size [{}]",
                target.email,
                serialized_size,
            );
            continue;
        ...
        {
            log::error!(
                "Failed to push to notification queue for email {}: {:?}",
                target.email,
                e
            );
        }
