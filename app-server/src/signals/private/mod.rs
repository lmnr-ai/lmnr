...
pub use processor_hook::check_and_push_signals;
pub use queue::push_to_signals_queue;
pub use spans::get_trace_structure_as_string;
    ...
    LLM_MODEL
        .get_or_init(|| {
            env::var("SIGNALS_LLM_MODEL")
                ...
                .filter(|v| !v.is_empty())
                .unwrap_or_else(|| crate::llm::default_model_for_provider(&llm_provider()))
        })
        .clone()
}
...
pub fn llm_provider() -> String {
    LLM_PROVIDER
        .get_or_init(|| crate::llm::resolve_provider_name())
        .clone()
}
