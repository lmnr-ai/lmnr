...
- Grep before adding a cache key or a `pub fn`. If an existing cache entry already holds the fact, read it instead of adding a second entry to keep in sync; two public functions with near-identical bodies are a design smell.
...
- Every query on a tenant-owned row filters on its owning scope (`WHERE id = $1 AND workspace_id = $2`), and tenant-scoped cache keys include the tenant — a cache hit skips the `WHERE` clause entirely.
...
- Don't change an existing signature as a side effect of a feature (flipped tuple order, `String` → `&str` plus a new lifetime, a widened param).
...
- A validation bound must trace to a column width, a shared constant or a product rule — no invented limits.
