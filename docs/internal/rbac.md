...
- `pii_checked Bool` on `spans` and `unique_content`: the redactor screened the row. Sparse storage makes an empty `*_redacted` ambiguous on its own (never screened vs. nothing found); the bit resolves it: checked + empty copy ⇒ raw is safe, checked + copy ⇒ show the copy, unchecked ⇒ unavailable. `false` for every pre-existing row, every `off` row, and every failure (RPC error, oversize, unparsable) — the two are indistinguishable in storage and need not be, since both fail closed. `SpanPii`/`PiiOutcome` live in `pii_redactor/mod.rs`; `redact_spans_in_place` is generic over `RedactTexts` so tests use a fake client.
- Dedup'd LLM messages (`unique_content`, group-scoped — `dedup-search.md`) get the same treatment per message; the span's own `*_redacted` columns are cleared when message hashes are present, since the view resolves those through the dict. `unique_content_dict` (recreated at frontend boot, `instrumentation.ts`) carries `content, content_redacted, pii_checked`. The legacy `deduped_content` table has no writer and no PII columns, so the masked branch skips its dict entirely: pre-migration rows are unavailable under mask.
