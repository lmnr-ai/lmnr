...
- `pii_checked Bool` on `spans` and `deduped_content`: the redactor screened the row. Sparse storage makes an empty `*_redacted` ambiguous on its own (never screened vs. nothing found); the bit resolves it: checked + empty copy ⇒ raw is safe, checked + copy ⇒ show the copy, unchecked ⇒ unavailable. `false` for every pre-existing row, every `off` row, and every failure (RPC error, oversize, unparsable) — the two are indistinguishable in storage and need not be, since both fail closed. `SpanPii`/`PiiOutcome` live in `pii_redactor/mod.rs`; `redact_spans_in_place` is generic over `RedactTexts` so tests use a fake client.
- Dedup'd LLM messages (`deduped_content`) get the same treatment per message; the span's own `*_redacted` columns are cleared when message hashes are present, since the view resolves those through the dict. `deduped_content_dict` (recreated at frontend boot, `instrumentation.ts`) carries `content, content_redacted, pii_checked`. Legacy `llm_messages_dict` content is unchecked and unavailable under mask.
- **Dedup self-heal**: the `s:` storage-presence key is skipped for rows whose redaction failed (`PiiOutcome::shared_row_failed`, in-process only — `off` rows are unchecked but not failed, and are stamped normally), so a failed `deduped_content` row is re-inserted by the next occurrence instead of needing a retry queue.
