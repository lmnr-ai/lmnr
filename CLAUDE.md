...
- `cargo check --features signals` and `cargo fmt` on `main.rs` both fail in OSS — the `signals` feature gates modules that live only in `lmnr-private`. Default-feature `cargo check` is the real gate; format leaf files individually with `rustfmt --edition 2024 <file>`. Full stub workaround list: `docs/dev/app-server.md`.
...
**Gotcha:** `dotenv` does NOT override already-exported env vars. A shell that exports `PORT` silently breaks the app-server HTTP listener (bind error is swallowed) and `next dev` (`EADDRINUSE`). Launch with explicit `PORT=8000 cargo r` / `PORT=3000 pnpm run dev`, or `env -u PORT`. Details: `docs/dev/app-server.md`.
...
- Full details (drizzle-kit quirks, snapshots, `POSTGRES_SCHEMA`): `docs/dev/database.md`.
...
- New data tables MUST follow the `InfiniteDataTable` split pattern (index/contents/controls/constants). Full patterns: `docs/dev/frontend-best-practices.md`.
...
`docs/dev/` holds detailed, hard-won working notes. **Before working in one of these areas, read the matching file** — it documents invariants, security boundaries, and gotchas that are not inferable from the code:

| File | Read when touching |
|---|---|
| `docs/dev/database.md` | Postgres migrations, `POSTGRES_SCHEMA`, name-sort collation |
| `docs/dev/sql-query-engine.md` | `query_engine/` validator (a security boundary), SQL editor schema/autocomplete, `/v1/sql/query` guards, rate limiting |
| `docs/dev/clickhouse-traces.md` | `traces_agg`/`traces_static`/`traces_v0`, spans query scoping, trace aggregation, async-insert tuning, traces-table filters, project data deletion |
| `docs/dev/dedup-search.md` | `shared_content`/`llm_messages` dedup, `spans_v0` reconstruction, Quickwit indexing/search |
| `docs/dev/ingestion.md` | OTLP `/v1/traces`, GenAI semconv parsing, trace metadata patches, input/output extraction, system-prompt extraction, checkpoints, 413s |
| `docs/dev/observability.md` | App-server self-tracing, Sentry layers/sampling |
| `docs/dev/mq-redis.md` | RabbitMQ queues + streams transport, Redis resilient connections, readiness probes |
| `docs/dev/auth.md` | Better Auth, OAuth providers, CLI device-flow auth, project API keys |
| `docs/dev/billing.md` | Tiers, usage warnings/hard limits, signal cost metering, custom model costs |
| `docs/dev/signals.md` | Signals, alerts, signal events (payloads, sorting, triggers) |
| `docs/dev/slack.md` | Slack OAuth broker + brokered self-hosted integration |
| `docs/dev/ai-features.md` | `getLanguageModel`, LLM provider config, Vercel AI SDK v7 |
| `docs/dev/analytics.md` | PostHog, Loops sync, self-hosted telemetry heartbeat |
| `docs/dev/labeling-queues.md` | Labeling queues (ClickHouse RMT items, dirty-state, push-to-dataset) |
| `docs/dev/debugger.md` | Debugger replay cache, debugger session blocks/timeline |
| `docs/dev/frontend-tables.md` | Data-table filters, advanced search, evaluations page, dashboards |
| `docs/dev/frontend-trace-view.md` | Trace view, span rendering/message parsing, ContentRenderer, rrweb replay |
| `docs/dev/frontend-app.md` | Settings pages, onboarding, base-path serving, render templates, SSE proxy routes, landing page, agent stubs |
| `docs/dev/frontend-best-practices.md` | Any new frontend component/table/store work |
| `docs/dev/app-server.md` | App-server env vars, local dev env quirks, signals-feature build stubs |
