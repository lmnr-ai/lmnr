...
- Reconstruction uses the `llm_messages_dict` COMPLEX_KEY_CACHE dictionary. It is **not** defined in the static migration — `CREATE OR REPLACE DICTIONARY` runs on every frontend boot from `ensureLlmMessagesDict` in `frontend/instrumentation.ts`, AFTER `clickhouse-migrations` finishes, so the `SOURCE(CLICKHOUSE(...))` clause can carry `CLICKHOUSE_USER` / `CLICKHOUSE_PASSWORD` / `CLICKHOUSE_DB` from env. The migration tool MD5-checksums each `.sql` file and refuses to start if a previously-applied file changes — baking creds into `43_llm_messages.sql` would break every boot the moment a credential rotates. `OR REPLACE` is also self-healing: if a deployment ever ended up with a credential-less dict (the original LAM-1578 bug), the next boot overwrites it. Earlier drafts used LEFT JOIN + `groupArray`, but CH cannot push the outer `trace_id` predicate through `GROUP BY`, so every single-trace view read aggregated all project messages. `dictGetOrDefault` keyed on `(project_id, trace_id, message_hash)` lets each span pay only for the hashes it actually references. Dictionary attributes cannot be `FixedString(N)` on CH 25.12 (fails `UNKNOWN_TYPE` at `CREATE DICTIONARY`) — `message_hash` is declared `String` in the dict and CH coerces the `FixedString(32)` hashes transparently at `dictGetOrDefault` call time. Dictionary refresh is configured via `LIFETIME(MIN 30 MAX 60)` so fresh inserts appear in lookups within ~1 minute.
