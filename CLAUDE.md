...
- Two telemetry flags are deliberately decoupled and must stay that way: `POSTHOG_TELEMETRY` gates product analytics (client-side feature tracking via `lib/posthog/`), `SELF_HOSTED_TELEMETRY` gates install-level reporting (`lib/telemetry/self-hosted.ts`: `instance:launched` / `instance:heartbeat` events keyed by the `self_hosted_instance` UUID). Don't couple them behind a single flag — they answer different questions. Both are also gated out entirely when `LAMINAR_CLOUD=true`. Install-level events have their own dedicated PostHog server client in `self-hosted.ts` — do NOT reuse `lib/posthog/server.ts` (it returns `null` when `Feature.POSTHOG` is off).
- Install-level dedup across replicas uses a single-row Postgres UPDATE with a cooldown interval (`UPDATE self_hosted_instance SET launched_at = NOW() WHERE launched_at IS NULL OR launched_at < NOW() - '1 hour'::interval RETURNING id`, and equivalent on `heartbeat_at` with 23h). The row is seeded by a migration, never inserted from app code — a missing row fails silently and skips the event. Launch uses 1h cooldown (covers rolling deploys), heartbeat uses 23h (daily-ish without clock-skew drift, ticks hourly). Each event type claims its own column so a recent heartbeat cannot suppress the next launch event (or vice versa) via a shared cooldown.
