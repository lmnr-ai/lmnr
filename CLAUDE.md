...
- The `spans_v0` view body ends with `SETTINGS optimize_move_to_prewhere = 0`. This works around a CH 25.12 analyzer bug: queries like `SELECT * FROM spans_v0(...) WHERE span_type = 'LLM' ORDER BY start_time DESC` fail with `AMBIGUOUS_COLUMN_NAME: span_type String / span_type UInt8` because the prewhere mover pushes the String predicate on the CASE-aliased `span_type` down onto the base `UInt8 spans.span_type` column with the same name. The bug reproduces whenever the view body contains an `arrayMap` over a base-table array column; an inner subquery renaming the base column does NOT help, and `enable_optimize_predicate_expression = 0` alone does NOT help. Disabling the prewhere move does — primary-key index use is unaffected (the `(project_id, trace_id)` PK lookup still runs).
