...
- The debounced target auto-save in `frontend/components/queue/queue.tsx` debounces on a `JSON.stringify({id, target})` key, not just the item id. `dirtyItemIds` is a Set — re-adding an already-dirty id is a no-op, so gating the debounce on id alone would coalesce repeated edits to the same item into a single save and silently drop every edit after the first flush. Any change here must preserve the content-sensitive key.
- The debounced target auto-save sends `isLabelled: false`. Approve and discard must (a) `abort()` any in-flight debounced fetch via `debouncedAbortRef` and (b) pre-set `lastSavedKeyRef.current = pendingSaveKey` to suppress a not-yet-fired timer. Without both, a delayed `isLabelled:false` PATCH can arrive after approve's `isLabelled:true` PATCH — RMT resolves by `updated_at`, so last-write-wins silently reverts the approval. Same reasoning applies to discard: a late re-insert with a fresh `updated_at` resurrects a row past its delete tombstone.
