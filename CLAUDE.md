...
- Plan step: "free" is the default. Selecting a paid tier and clicking Continue does `window.location.href = /checkout?lookupKey=...`. `app/checkout/page.tsx` hardcodes Stripe's `success_url` / `cancel_url` to `/workspace/${workspaceId}?tab=billing`, so the user does NOT return to `/onboarding` — the paid-tier branch must therefore mark onboarding complete before redirecting (track `onboarding:completed` + fire-and-forget `DELETE /api/onboarding/state` with `keepalive: true`). Non-subscription environments (`Feature.SUBSCRIPTION` off, read via `useFeatureFlags()`) short-circuit to a single "Finish" button.
