...
- Snippet screenshots must be captured from the **full-page trace route** `/project/<pid>/traces/<traceId>?spanId=<llmSpanId>` (NOT the drawer-mode `/traces?traceId=<tid>` — that route is narrower and portrait-oriented). Before capture: collapse the project sidebar (click "Toggle Sidebar"), select an LLM span via `spanId` in the URL so the right-hand span panel shows Span Input/Output (System/User messages, tool calls), and shoot at the 1512x982 viewport with no `fullPage` flag. Reference integrations (`browser-use.png`, `claude-agent-sdk.png`, `lang-chain.png`, `vercel-ai-sdk.png`) were captured at retina (~2050x1750) but the 1512x982 shots render equivalently once Next.js serves them.
