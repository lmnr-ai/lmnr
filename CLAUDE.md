...
- AI generation for render templates follows the same pattern as SQL generation: route at `app/api/projects/[projectId]/render-templates/generate/route.ts`, action at `lib/actions/render-template/generate.ts`, prompts at `lib/actions/render-template/prompts.ts`. Uses `getLanguageModel("default")` + `generateObject` with a `{success, result, error}` schema. The system prompt carries the Laminar Tailwind style guide so generated UIs visually match the platform; keep that style guide in sync with the actual palette/spacing if the design system changes meaningfully.
- `ManageTemplateDialog` (`frontend/components/ui/template-renderer/manage-template-dialog.tsx`) is a shared form UI consumed both from the inline template picker (`components/ui/template-renderer/index.tsx`) and from project settings (`components/settings/render-templates/`). Both callers must wrap it in a `FormProvider` whose form shape matches `ManageTemplateForm` — the dialog uses `useFormContext` and will crash without a provider. The settings wrapper in `render-template-dialog.tsx` fetches the full template on open and resets the form; the inline picker already has the template loaded via SWR + `fetchTemplate`.
