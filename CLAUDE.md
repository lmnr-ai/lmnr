...
- **Single dedup path on the consumer.** `process_span_messages` mints a dedup via `build_dedup` for any recordable span that arrived without one (legacy producers, `routes/spans.rs::create_span` which still bypasses `publish_span_messages`, or any future bypass). After this normalisation every recordable LLM span with array input has a dedup, and `build_dedup_batch` is the only batch-builder. This avoids the mixed-flush data-loss footgun where a `None`-dedup span batched alongside pre-processed spans would otherwise be silently treated as having empty hashes. Non-LLM / non-array-input spans correctly remain `None` and emit empty hashes — they have no dedup work to do.
