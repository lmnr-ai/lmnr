...
- The Redis "seen" marker is keyed `"m:" + project_id.as_bytes() + hash.as_bytes()` (50 bytes binary, mapped to a String via Latin-1 byte-as-char since the `Cache` trait is String-keyed). TTL is 3600s — a short-lived circuit breaker, not durable state; when Redis is unreachable we still insert into `llm_messages` and let ReplacingMergeTree dedup on merge. Do NOT try to encode the key as hex/base64 — it'd double the size and change the uniqueness domain.
