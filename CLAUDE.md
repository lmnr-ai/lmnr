...
- `frontend/app/onboarding/page.tsx` is the multi-step wizard entry. It runs in a FormProvider (react-hook-form) whose values live in `components/onboarding/types.ts::OnboardingFormValues`. Each step is a sibling file under `components/onboarding/steps/` wrapped in the shared `step-shell.tsx` (centered, scrollable card with progress dots + back/next).
- Resume state (`{workspaceId, projectId, step}`) persists in a 1-day cookie via the `onboarding-state` server-action helpers in `lib/actions/onboarding/index.ts`. The cookie is `httpOnly` + `secure` (production) so workspace/project IDs aren't readable via `document.cookie` — it's only consumed server-side in `app/onboarding/page.tsx` and the POST/DELETE route handlers, never by client JS. `app/api/onboarding/state/route.ts` wraps POST/DELETE for the client. The page validates the cookie against `projects` ⨝ `membersOfWorkspaces` before resuming and clears stale cookies so deleted workspaces don't trap the user.
- The "already has workspaces" legacy redirect to `/projects` fires only when `count > 0 AND !resumeState AND slack query param is undefined` — otherwise returning from Slack OAuth or Stripe checkout would bounce users out of onboarding.
- Creating a workspace with `isFirstProject: true` (via `POST /api/workspaces`) already auto-creates the **Failure Detector** signal, its trigger, and an email alert target for the creator. The Signals step must skip re-creating Failure Detector. The Notifications step opt-out path must DELETE existing email report targets (not create new ones).
...
- Plan step: "free" is the default. Selecting a paid tier and clicking Continue does `window.location.href = /checkout?lookupKey=...`. `app/checkout/page.tsx` hardcodes Stripe's `success_url` / `cancel_url` to `/workspace/${workspaceId}?tab=billing`, so the user does NOT return to `/onboarding` — the paid-tier branch must therefore mark onboarding complete before redirecting (track `onboarding:completed` + fire-and-forget `DELETE /api/onboarding/state` with `keepalive: true`). Non-subscription environments (`Feature.SUBSCRIPTION` off) short-circuit to a single "Finish" button.
...
- `handleSaveNotifications` must handle BOTH directions of the email checkbox on re-entry — opt-out (DELETE via `/api/workspaces/:id/reports`) and opt-back-in (POST to the same route, which `optInReport` serves idempotently). Track opt-outs in `optedOutReportIds` so a Back → re-check → Continue round-trip re-creates the auto-created email targets instead of leaving them deleted.
