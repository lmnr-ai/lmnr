...
- LLM span `input` is stored structurally deduplicated: each message in the JSON array is BLAKE3-256 hashed (canonical JSON with sorted object keys so field-order-only differences collapse), emitted as a row in `llm_messages` (ReplacingMergeTree, `ORDER BY (project_id, trace_id, message_hash)`, trace-scoped — same content across two traces is two rows), and referenced from the span via `spans.input_message_hashes Array(FixedString(32))` while `spans.input` is stored empty. The `spans_v0` view reconstructs the original JSON array via `arrayMap(h -> dictGetOrDefault('llm_messages_dict', 'content', (project_id, trace_id, h), 'null'), input_message_hashes)`; when `input_message_hashes` is empty it falls through to `input`. The tool-span / non-LLM `input` is never touched. Content stored in `llm_messages` is run through `sanitize_string` (same as the non-dedup `CHSpan::from_db_span` path) so CH-unsafe characters are consistently stripped; the BLAKE3 hash is computed over the pre-sanitized canonical JSON so dedup identity is stable.
- Reconstruction uses the `llm_messages_dict` COMPLEX_KEY_CACHE dictionary defined in `frontend/lib/clickhouse/migrations/42_llm_messages.sql`. An earlier draft used a LEFT JOIN + `groupArray` aggregation, but CH cannot push the outer `trace_id` predicate through `GROUP BY` boundaries, so every single-trace view read aggregated all project messages. `dictGetOrDefault` keyed on `(project_id, trace_id, message_hash)` lets each span pay only for the hashes it actually references. Dictionary attributes cannot be `FixedString(N)` on CH 25.12 (fails `UNKNOWN_TYPE` at `CREATE DICTIONARY`) — `message_hash` is declared `String` in the dict and CH coerces the `FixedString(32)` hashes transparently at `dictGetOrDefault` call time. Dictionary refresh is configured via `LIFETIME(MIN 30 MAX 60)` so fresh inserts appear in lookups within ~1 minute.
- The underlying `spans` column for span type is named `span_kind` (UInt8) — NOT `span_type`. Renamed in migration `42_llm_messages.sql` to break a CH 25.x analyzer ambiguity: when the physical column AND the `spans_v0` projection alias were both named `span_type`, the analyzer auto-renamed the view alias to `t` and threw `AMBIGUOUS_COLUMN_NAME: span_type String / span_type UInt8` under `WHERE span_type='LLM' ORDER BY start_time DESC LIMIT N`. The `spans_v0` view still exposes the string-cast projection as `AS span_type`, so every frontend / query-engine / user-facing reference to `span_type` continues to work unchanged — only the raw ingestion side (`CHSpan.span_kind: u8` in `app-server/src/ch/spans.rs`) and direct queries against the raw `spans` table (`evaluation_datapoints_v0`) use the new name. `spans.span_type` does NOT exist any more; if you write a new view / materialized view / mutation that reads the raw `spans` table, you MUST reference `span_kind`, and any `WHERE span_type = N` against the raw table will fail.
- Dropping and re-adding the `spans_no_io_by_start_time` PROJECTION is mandatory when renaming `spans` columns: CH errors `Cannot apply mutation because it breaks projection` on `RENAME COLUMN` if the column appears in any projection body. Migration 42 drops the projection, renames the column, then re-adds and materializes the projection with the new column name. Any future `spans` column rename must follow the same drop → rename → re-add → materialize sequence.
- Dedup affects `spans.size_bytes` and the workspace usage counter: `process_span_messages` in `app-server/src/traces/processor.rs` rewrites each dedup'd span's `size_bytes` AFTER `build_dedup_batch` as `original_size - estimate_json_size(input) + 32 * num_hashes`, so billing reflects what we actually store (32-byte `FixedString` hashes, not the full JSON array). The rewrite happens before `CHSpan::from_db_span` (so the CH `size_bytes` column is correct) AND before `update_workspace_bytes_ingested` (so workspace limits / billed bytes reflect dedup savings). The `llm_messages` row content itself is NOT currently counted toward workspace bytes — dedup's whole point is that the one-time storage of unique messages is amortized across the many spans that reference them.
...
- Redis key is `m:{project_id_simple}:{trace_id_simple}:{hex(hash)}` with TTL 1h — it is ONLY a hot-path "seen recently" filter that skips re-emitting identical rows on every agent step. It MUST include `trace_id` because `llm_messages` is trace-scoped (`ORDER BY (project_id, trace_id, message_hash)`): a project-scoped Redis key would suppress the insert of a message already seen in a different trace of the same project, leaving that second trace's span with `input_message_hashes` pointing at rows that do not exist for its `trace_id`, which the view's `LEFT JOIN ... GROUP BY trace_id` reconstructs as an empty string. It is best-effort: on `cache.exists()` error we always insert and rely on ReplacingMergeTree to collapse duplicates on merge. The `mark_seen` call happens only AFTER a successful `llm_messages` insert; on insert failure `unmark_seen` is called and the whole batch returns `HandlerError::transient` so it is retried. Never flip the order — stamping Redis before the insert can leave a permanent "seen" marker for a row that never landed.
- Ingest order in `process_span_messages` is strict: build dedup batch → insert `llm_messages` → stamp Redis → insert `spans`. Spans point at message hashes that may not yet be queryable if the messages insert is racing behind, but `spans_v0` is read with `FINAL` semantics and ReplacingMergeTree merges on `(project_id, trace_id, message_hash)` so late-arriving duplicates are fine.
- Within one flush, `build_dedup_batch` deduplicates `(project_id, trace_id, hash)` triples via a `HashSet` so two spans in the same trace referencing the same new message emit one `CHLlmMessage`, not two. The HashSet key MUST match the `llm_messages` ORDER BY — a batch can mix spans from multiple projects, and two projects sharing a `trace_id` would otherwise suppress the second project's insert, leaving its spans pointing at rows that don't exist for its `(project_id, trace_id)` scope.
