...
- Project-level toggle stored as `projects.remove_pii BOOLEAN` (Postgres) — NOT a per-span attribute. Setting it on a project routes every span ingested for that project through the `pii-redactor` gRPC service before storage. The flag travels on the same row as `ProjectWithWorkspaceBillingInfo` (Rust) which is cached in Redis under `project:{project_id}`, so the redaction-decision lookup is free after first batch.
- Frontend toggle: `components/settings/pii-redaction.tsx`, mounted in the General tab. Pro-tier gated in BOTH the UI (greyed `Switch` + "Pro plan required" badge) AND the server action (`updateProjectRemovePii` in `lib/actions/project/index.ts` rejects non-Pro/Enterprise tiers with 403). The server gate is the source of truth; the UI gate is purely UX.
- After flipping the flag, the server action calls `deleteProjectWorkspaceInfoFromCache(projectId)` to invalidate the app-server's cached `ProjectWithWorkspaceBillingInfo` row. Without the invalidation, ingestion would keep using the cached old `remove_pii` value until TTL.
