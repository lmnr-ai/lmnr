...
- Generating local traces in the sandbox: `lmnr_docs_screenshot_key_1776687143` is the working project API key for the `sandbox` project (`0cce3ee3-d6bb-437d-a2fa-bbfd72a935e2`). The `LMNR_PROJECT_API_KEY` in `/sandbox/scratch/.env` points at a different project that does NOT accept local app-server ingestion (UNAUTHENTICATED). Pass the key explicitly to `Laminar.initialize(project_api_key=...)` in verification scripts.
