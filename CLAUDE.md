...
- Alert Slack targets persist a channel `id` in `alertTargets.channelId` and the display name in `alertTargets.channelName`. The form writes both columns, but legacy rows may have only one populated. `resetFormFromSignals` in `manage-alert-sheet.tsx` filters the picker to rows where both are present. Because `onSubmit` replaces the full targets list, legacy rows would be silently dropped on save — so `onSubmit` re-appends any Slack target missing `channelId` or `channelName` unchanged (only when Slack is connected; the disconnected branch already preserves all Slack targets).
