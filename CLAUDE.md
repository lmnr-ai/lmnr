...
## LLM Input Message Dedup (`llm_messages`)

- Agent traces repeat most of the conversation on every step (step k's input is step k-1's input plus one new message), so we dedup LLM span inputs into a content-addressed `llm_messages` ClickHouse table (migration `42_llm_messages.sql`). Each message is canonicalized (sorted-keys JSON) and hashed with BLAKE3-256; first-seen hashes are inserted and the span row stores the ordered hash array in `spans.input_message_hashes Array(FixedString(32)) CODEC(ZSTD(3))`. The `spans_v0` view reconstructs `input` as a JSON array via LEFT JOIN with `groupArray(message_hash)` / `groupArray(content)` on `llm_messages`, selected by `{project_id:UUID}` — a per-trace correlated subquery would fire one SELECT per message slot and is the wrong shape; join per-trace once and `indexOf` into the arrays.
- Dedup keying is `(project_id, trace_id, message_hash)`, NOT project-wide. Traces are usually short-lived; a global key means a cross-trace cache hit makes the content unreadable once old traces age out of retention. `llm_messages` is `ReplacingMergeTree` ordered by those three columns so any duplicate rows that slip past the Redis "seen" marker collapse on merge.
- Dedup runs in `app-server/src/traces/llm_message_dedup.rs` between `CHSpan` build and the span insert in `process_span_messages`. Messages MUST land in ClickHouse before the span row, otherwise the view returns `input = []` for the window where span exists but messages don't. On `llm_messages` insert failure we delete the Redis "seen" markers we just set and requeue the batch — without the rollback, the retry would treat those hashes as already-inserted and the view would permanently reconstruct `input` as empty. Only LLM spans whose `span.input` is a JSON array participate; non-array inputs and non-LLM spans flow through unchanged with empty `input_message_hashes` and raw `input`.
- `CHSpan` struct field order does NOT need to match DB column order — the `clickhouse` crate emits `INSERT INTO table(fields) FORMAT RowBinary` with named fields. Adding `input_message_hashes` as the last field of the struct is safe even though the column lives in the middle of the schema. The empty-vec default (`Vec::new()` in `from_db_span`) matters because for non-LLM spans the dedup step never touches the field.
- The Redis "seen" marker is keyed `"m:" + project_id.as_bytes() + trace_id.as_bytes() + hash.as_bytes()` (66 bytes binary, mapped to a String via Latin-1 byte-as-char since the `Cache` trait is String-keyed). `trace_id` must be in the key because `llm_messages` is keyed by `(project_id, trace_id, message_hash)` — a project-only key makes the marker report "seen" for a second trace and skips the insert, leaving the view unable to reconstruct `input` for that trace. TTL is 3600s — a short-lived circuit breaker, not durable state; when Redis is unreachable we still insert into `llm_messages` and let ReplacingMergeTree dedup on merge. Do NOT try to encode the key as hex/base64 — it'd double the size and change the uniqueness domain.
- The `clickhouse-migrations` tool tracks file md5 in the `_migrations` table. If an environment has a different prior copy of an already-numbered migration (e.g. a partial apply before a rewrite), the checksum mismatch aborts the migration — reconcile by deleting the `_migrations` row for that version and re-applying the new file's statements one at a time via the HTTP API (POST, not GET; CH HTTP refuses multi-statement files with "Multi-statements are not allowed").
