...
## LLM Input Message Dedup (`llm_messages` / `spans_v0`)

- LLM span input messages are content-addressed: each message is canonicalized (JSON with recursively sorted keys), BLAKE3-256 hashed, and stored once per `(project_id, trace_id)` in the ClickHouse `llm_messages` table. Spans hold only `input_message_hashes: Array(FixedString(32))` instead of the full input JSON; the original `spans.input` column is cleared for LLM rows when hashes are populated. Migration: `frontend/lib/clickhouse/migrations/42_llm_messages.sql`.
- Ingestion flow (`app-server/src/traces/llm_message_dedup.rs` + `processor.rs`): for each LLM span, compute hashes → Redis `SETNX m:{project_id_hex}{trace_id_hex}{hash_hex}` with 1h TTL → only insert `llm_messages` rows for hashes we "won" → insert `llm_messages` **before** spans. If `llm_messages` insert fails, release the Redis markers and requeue. A circuit breaker falls back to embedding full input in `spans.input` when Redis is unreachable — do NOT remove this fallback. The Redis dedup key MUST include `trace_id` because `llm_messages` is ordered by `(project_id, trace_id, message_hash)`; a project-only key would let a hash seen in one trace suppress the insert for another trace, leaving that trace's spans pointing at missing rows and rendering blank input in `spans_v0`.
- **`spans_v0` is a ClickHouse parameterized view.** Call it as `spans_v0(project_id='<uuid>')`, NOT `SELECT ... FROM spans_v0 WHERE project_id = ?` with a bound param — the latter 500s with "Unknown expression identifier". The `{project_id:UUID}` placeholder inside the view only binds when passed positionally via the parameterized-view syntax.
- **Correlated subqueries do NOT work inside ClickHouse parameterized views** (tested on 25.12). `(SELECT content FROM llm_messages WHERE ... AND trace_id = s.trace_id AND message_hash = h)` inside `arrayMap` fails with "Not found column h in block" and even with `allow_experimental_correlated_subqueries=1` fails with "can't find correlated column". The working pattern in `42_llm_messages.sql` is a LEFT JOIN to a grouped projection: `SELECT trace_id, groupArray(message_hash) AS hs, groupArray(content) AS cs FROM llm_messages WHERE project_id = {project_id:UUID} GROUP BY trace_id`, then reconstruct via `arrayMap(x -> m.cs[indexOf(m.hs, x)], s.input_message_hashes)`. The `if(length(s.input_message_hashes) > 0 AND notEmpty(m.hs), ..., s.input)` fallback preserves legacy rows without hashes.
- The dedup is transparent to the frontend: span-view parsers (`parseGenAIMessages`, OpenAI/Anthropic/LangChain detectors in `frontend/components/traces/span-view/messages.tsx`) read `input` from `spans_v0` and never see the hashes. When adding a new endpoint that reads span input, query `spans_v0(project_id=...)` — querying raw `spans.input` returns empty strings for hashed LLM rows.
