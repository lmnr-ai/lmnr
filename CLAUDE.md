...
- Dedup affects `spans.size_bytes` and the workspace usage counter: `process_span_messages` in `app-server/src/traces/processor.rs` rewrites each dedup'd span's `size_bytes` AFTER `build_dedup_batch` as `original_size - estimate_json_size(input) + 32 * num_hashes + span_content_bytes`, where `span_content_bytes` is the total size of the `llm_messages.content` rows this span caused to be newly inserted this batch. Messages already seen (Redis hit, or emitted earlier in the batch) contribute zero — a shared message is charged exactly once, to the span that first inserted it, and subsequent referrers only pay the 32-byte hash. This keeps the per-span `size_bytes` column faithful to CH storage written on behalf of that span and preserves the core dedup win (repeated agent-step messages don't pay full freight on every step). The rewrite happens before `CHSpan::from_db_span` (so the CH `size_bytes` column is correct) AND before `update_workspace_bytes_ingested` (so workspace limits / billed bytes reflect dedup savings). `build_dedup_batch` returns `span_content_bytes: Vec<usize>` aligned with `span_hashes` for this attribution.
