...
- Single-evaluation and shared-evaluation pages render `frontend/components/evaluation/distribution-chart/`, a tabbed chart with one tab per score key. Score types (`binary` / `discrete` / `continuous`) and bins are inferred server-side in `frontend/lib/actions/evaluation/utils.ts` (`analyzeScore`) and returned on the `/stats` endpoint as `allScoreAnalyses: Record<name, EvaluationScoreAnalysis>`. Binning rules: binary → 2 bins (0,1) with implicit pass threshold 1; discrete → 1 bin per integer in [min,max] (fills gaps); continuous → 10 bins over `[CONTINUOUS_LOWER, CONTINUOUS_UPPER]` (0..1 by default), with bounds auto-expanded to cover any out-of-range scores so `stats.count` always matches the sum of bar heights. Bin-edge label decimals scale with bin width.
- Pass thresholds for discrete/continuous scores are per-score and user-configurable. There is NO storage for them today — `getEvaluationStatistics` / `getSharedEvaluationStatistics` pass an empty `passThresholds` map to `analyzeScore`, so non-binary scores fall back to neutral rendering (no green/red split, no threshold line). Wiring storage requires a new `score_config` table and settings UI; do not inline the threshold into `evaluations` or `scores`.
- Bin click → URL filter contract: `filtersForBin` (`distribution-chart/utils.ts`) builds `{column: "score:<name>", operator, value}` JSON strings. Single-value bins use `Operator.Eq`; range bins use `Operator.Gte` + `Operator.Lt` for non-last continuous bins and `Operator.Lte` for the last. Clicking the currently-selected bin toggles the filter off. `isBinSelected` drives the dimmed-bar highlight in the chart.
- Collapse state persists per page-context via localStorage key `lmnr.distribution-chart.collapsed.<persistKey>`. Callers set `persistKey={`eval:${evaluationId}`}` and `shared-eval:${evaluationId}` so single and shared views have independent memory. The `DistributionChart` reads/writes storage in a try/catch; storage-disabled browsers just lose persistence across reloads.
- `DistributionChart` keeps the selected-tab valid in render-phase (`effectiveSelected = selectedScore && scoreNames.includes(selectedScore) ? selectedScore : scoreNames[0]`) rather than via `useEffect` + `setState` — the latter triggers `react-hooks/set-state-in-effect`. If a score disappears after a filter change, the derived value falls back to the first available name without an extra render.
- Comparison mode (`?targetId=...`) still uses the old `ScoreCard` + `CompareChart` layout. The new `DistributionChart` renders only when `targetId` is absent. Shared eval has no comparison UI, so it always renders `DistributionChart`.
