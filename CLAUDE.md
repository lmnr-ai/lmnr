...
- `MessageQueueTrait::publish_best_effort` is the call to use for **secondary, reproducible** publishes from the consumer hot path (Quickwit indexer in `quickwit/producer.rs`; signals queue in the private fork). The Rabbit impl tightens the `publish` retry envelope to ~3s (`RABBITMQ_BEST_EFFORT_PUBLISH_BUDGET_SECS`, max_interval 500ms) AND wraps the whole call in `tokio::time::timeout` as a hard ceiling. Callers MUST treat the error as drop-and-continue. Why: under broker high-watermark flow control, the primary `publish` retries for up to 60s; if the consumer is holding an unacked message while that retry blocks, prefetch slots stay pinned and the queue stops draining — the broker's own back-pressure feeds the cascade. Do NOT use `publish_best_effort` for the primary OBSERVATIONS / SPANS_DATA_PLANE publish in `traces/producer.rs` — those payloads are not reproducible.
