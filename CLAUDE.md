...
- `@/lib/api/api-handler` — server-side `apiHandler<P>(handler)` wraps Next.js route handlers. First calls `unstable_rethrow(error)` so Next's internal control-flow errors (`NEXT_REDIRECT`, `NEXT_NOT_FOUND`, etc. thrown by `redirect()` / `notFound()`) propagate to the runtime instead of being swallowed as 500s. Returns 400 (`prettifyError`) for `ZodError` WITHOUT reporting to Sentry (user-input validation errors are expected 400s — capturing them pollutes error tracking). For all other errors, reports to Sentry with `http.method` / `http.route` / `source: "apiHandler"` tags and returns 500. Use for any new route handler instead of hand-rolled try/catch.
