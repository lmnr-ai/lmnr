...
- `span.input = None` on the wire when a plan was produced — EXCEPT for root spans (`parent_span_id.is_none()`). Root spans keep their full `input` because `TraceAggregation::from_spans` reads it for `root_span_input` (truncated preview rendered in the trace list). Root spans are 1 per trace; the wire savings come from the long tail of nested LLM spans.
