...
## Session-view Media Player

The session-view media player (`frontend/components/traces/session-view/session-media-player/`) plays each trace in a session as a YouTube-style chapter — some chapters are rrweb browser-session replays, others are LLM-span image sequences. The playhead is canonical absolute-epoch-ms in the session-view store and all surfaces derive their local offsets from it.

- rrweb-player v2.0.0-alpha.18 has NO in-place event swap — switching chapters requires `$destroy()` + new `rrwebPlayer(...)`. `browser-session-surface.tsx` maintains an LRU cache (`MAX_CACHED_CHAPTERS=4`) of decoded event arrays since a single chapter can be >10MB after gunzip.
- `seekRequest` in the store is `{epochMs, token}` where `token` increments every call — this guarantees reference inequality so `rawStore.subscribe((state, prev) => ...)` can detect a fresh seek even when `epochMs` coincidentally matches the previous value.
- rrweb emits `ui-update-current-time` during playback which surfaces forward to `setPlayheadEpochMs`. Guard the return trip (`setPlayheadEpochMs → seekRequest → goto`) with `Math.abs(seekRequest.epochMs - lastSurfaceEpochMsRef.current) < 50` or the player enters a goto/ui-update loop.
- `mediaPanelOpen` must NOT be persisted (exclude from `partialize`). The player allocates heavy rrweb DOM + event arrays; auto-opening on fresh mount is jarring. Same rationale as `spanPanelOpen` in the trace-view store.
- `images-surface.tsx` uses a `setInterval(..., 42)` playback ticker. Gate the effect on `isPlaying` (select it into deps and early-return when false) — running a 24Hz no-op timer when the user has the chapter paused wastes CPU across every open session view.
- Image preload state must be populated per-image: `setPreloaded(prev => prev.has(url) ? prev : new Set(prev).add(url))` inside each `onload`. Do NOT `Promise.all(images.map(...))` then commit the whole set — one slow or errored image blocks all later frames from ever appearing.
- Chapter auto-advance: the orchestrator (`index.tsx`) must handle `playhead > activeChapter.endTimeMs` by locating the next chapter with `startTimeMs > playhead` and `seekTo(next.startTimeMs)`. Without this, playback stalls at the tail of every chapter.
- Images-surface playback ticker boundary: the orchestrator's chapter-containment check uses `<=`, so a playhead *exactly* at `endTimeMs` still reads as "inside this chapter" and auto-advance never fires. The ticker at end-of-chapter must nudge the playhead PAST `last` (e.g. `last + 1`) to push it into the gap/next-chapter region. If there's no next chapter, pause instead. Don't clamp to `last`.
- Timeline playhead boundaries: segments match with `>= start && <= end` (inclusive); gaps match with `> start && < end` (strict). Together they cover the timeline without double-claiming the boundary instants.
...
- Per-tick re-renders from the playhead: timeline segments / gaps should derive the `playheadLeftPercent` (or `null` when outside their own time domain) INSIDE the zustand selector, not select `playheadEpochMs` then derive in `useMemo`. During playback `playheadEpochMs` changes at 24 Hz; with the derivation in `useMemo`, every segment re-renders every tick. With derivation inside the selector + `shallow` equality, a null-to-null transition is a no-op, so only the segment currently hosting the playhead re-renders. Consolidate all store reads for the segment into ONE `useSessionViewStore(selector, shallow)` call — separate selector calls each register a subscription and multiply re-renders per tick.
- `browser-session-surface.tsx` MUST always render the rrweb container `<div ref={containerRef}>` — overlay the loader / empty-state on top, don't conditionally swap them in. `ensureChapter()` toggles `setIsLoading(true)` → awaits → `setIsLoading(false)` → calls `mountOrSwap()` synchronously. If the container were conditionally rendered on `!isLoading`, the batched `setIsLoading(false)` wouldn't have committed yet when `mountOrSwap` reads `containerRef.current`, so it'd be null and the player would never mount on cache miss.
