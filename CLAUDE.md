...
- Hashing + Redis-existence checks ALSO run on the producer (HTTP / gRPC ingest in `publish_span_messages`) BEFORE the message hits Rabbit. Already-seen LLM input messages are stripped from `span.input`; the wire payload carries an `LlmInputDedupPlan` (`hashes`, `new_indices`, `new_contents`) instead. Wire savings grow linearly with conversation history depth, which is the point.
- `RabbitMqSpanMessage` carries two new fields with serde defaults: `pre_processed: bool` (consumer skips `parse_and_enrich_attributes` + `convert_span_to_provider_format` when true) and `input_dedup: Option<LlmInputDedupPlan>` (the producer's hash + Redis-status verdict).
...
- **Single dedup path on the consumer.** `process_span_messages` mints a plan via `build_producer_plan` for any recordable span that arrived without one (legacy producers, `routes/spans.rs::create_span` which still bypasses `publish_span_messages`, or any future bypass). After this normalisation every recordable LLM span with array input has a plan, and `build_dedup_batch_from_plans` is the only batch-builder. This avoids the mixed-flush data-loss footgun where a `None`-plan span batched alongside pre-processed spans would otherwise be silently treated as having empty hashes. Non-LLM / non-array-input spans correctly remain `None` and emit empty hashes — they have no dedup work to do.
- `build_dedup_batch_from_plans` is hardened against malformed plans: `plan.hashes.get(pos as usize)` and `plan.new_contents.get(i)` are bounds-checked with `else continue`, so a corrupt-on-deserialise plan or a future producer bug skips the bad entries instead of panicking and looping the entire flush on requeue.
