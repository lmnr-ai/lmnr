...
- The "already has workspaces" legacy redirect to `/projects` fires only when `count > 0 AND no saved cookie AND slack query param is undefined`. Whenever a saved cookie exists (even with stale/foreign data), render the wizard so its mount-effect can refresh the cookie with the current session before any further navigation.
