...
- Ingest order in `process_span_messages` (LAM-1609): build dedup batch synchronously, then run the trace upsert, the `llm_messages` insert (+ `mark_seen` on success), and the `spans` insert concurrently via a 3-way `tokio::join!`. The strict `llm_messages → mark_seen` ordering still holds inside the messages branch — never flip it. Within one flush, `build_dedup_batch` dedupes `(project_id, trace_id, hash)` triples via HashSet — the key MUST match `llm_messages` ORDER BY since a batch can mix projects.
- Parallelizing `llm_messages` and `spans` accepts a brief inconsistency window: a span row can land in CH before its referenced messages do. Reconstruction via `llm_messages_dict` returns `'null'` for missing hashes (`dictGetOrDefault`), so readers see partial input for ms-scale until both inserts complete. If `llm_messages` fails but `spans` succeeds, the batch still returns `HandlerError::transient` so the whole flush retries — `spans` is RMT-keyed so the re-insert collapses cleanly. Net effect: orphaned-hash spans exist only between the failed insert and the next retry tick.
