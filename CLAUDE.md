...
- Server-side captures from NextAuth callbacks (e.g. `auth:user_created` in `lib/auth.ts`) must `await client.shutdown()` after `client.capture(...)` — `posthog-node` batches by default and the serverless request can terminate before the batch flushes, dropping the event. Wrap the whole block in try/catch so analytics failures cannot break login.
