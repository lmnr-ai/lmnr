...
- Workspace grouping is registered server-side via `posthog.groupIdentify({ groupType: "workspace", groupKey: workspace.name, properties: { name, id }, distinctId: user.email })` in `app/workspace/[workspaceId]/page.tsx` and `app/project/[projectId]/layout.tsx`, alongside the existing `posthog.identify` call. Group key is `workspace.name` (readable in the PostHog UI) with `{ name, id }` as traits — do NOT key by id, PostHog group filters display the key directly. Note: server-side `groupIdentify` only registers/updates the group entity's properties; it does NOT make subsequent client-side `track(...)` events automatically tagged with the workspace group. If you need client-side events associated with the group, call `posthog.group(...)` from a client component instead.
