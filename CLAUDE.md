...
- **Every onboarding-completion DELETE of `/api/projects/<id>/onboarding/state` must gate the next navigation on `res.ok`, not just on the fetch promise resolving.** `fetch` only throws on network errors — any HTTP status (401 on session expiry, 500 from the route handler) resolves normally. Navigating into the `(authenticated)` tree with a live cookie loops via the layout gate → `/onboarding`. This applies to `finishFreeTier` in `components/onboarding/use-onboarding-actions.ts` (returns boolean; the free-tier branch propagates it and toasts on failure) and to `PaidFinalize` in `components/onboarding/index.tsx` (the Complete-button handler reads `res.ok`, releases `isFinishing` + toasts on failure so the user can retry, only calls `router.replace(...)` on success). The server-side stale-cookie clear (cookie references a project the user no longer has access to) is handled separately via `redirect("/api/onboarding?to=/projects")` in `app/(auth)/onboarding/page.tsx` — `/api/onboarding` is the only allowed cookie write that isn't project-scoped, because at that point the project from the cookie can't be authz'd against `proxy.ts`.
- There is no mount-POST in the current wizard. Step-state writes happen only after a successful step save (`persistOnboardingStep` in `use-onboarding-actions.ts`), so completion handlers can DELETE without worrying about a racing Set-Cookie. If you reintroduce a mount-POST in the future, the DELETE in `finishFreeTier` and `PaidFinalize` must await its settle (success or failure) before issuing the DELETE — otherwise the POST's `Set-Cookie` would land after the DELETE, resurrect the cookie, and bounce the user back to `/onboarding` via the `(authenticated)` layout gate.
...
- **Welcome email is sent from the onboarding-completion DELETE handler**, not from the NextAuth `jwt` callback. `app/api/projects/[projectId]/onboarding/state/route.ts::DELETE` calls `sendWelcomeEmail(session.user.email)` after `clearOnboardingState`, gated on `isFeatureEnabled(Feature.SEND_EMAIL)`. Two call sites hit this DELETE: `finishFreeTier` (cloud free path) and `PaidFinalize.handleComplete` (cloud paid path, after Stripe). Sending earlier (e.g. on first sign-in) would email users who bounce out of onboarding before creating a workspace. **Idempotency:** the email send is gated on `(await getOnboardingState())?.userId === session.user.id` BEFORE `clearOnboardingState()` runs — a `PaidFinalize` remount, a browser retry on a partial response, or any future second DELETE finds no cookie and skips the send. `clearOnboardingState()` itself is already idempotent (cookie delete). OSS does not write the onboarding cookie and therefore never sends the welcome email; that's intentional — OSS deployments rarely set `RESEND_API_KEY`, and any side-channel "send for OSS too" hack would have to bypass the same guard that protects the cloud paths from duplicates.
