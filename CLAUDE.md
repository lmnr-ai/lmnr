...
- Resume defaults (`lib/actions/onboarding/resume-defaults.ts::loadOnboardingResumeDefaults`) re-derive form defaults (`workspaceName`, `selectedTemplateNames`, `subscribedReportIds`, `slackConnected`, `selectedTier`) from the DB on every page load when a workspace/project are bound, so refreshing or revisiting steps shows the user's actual current state. The function is split into per-slice loaders (`loadWorkspaceContext`, `loadSelectedTemplateNames`, `loadSlackConnected`, `loadSubscribedReportIds`) composed with `Promise.all` — keep that shape when adding fields so the per-slice queries can run in parallel. Save handlers in `use-onboarding-actions.ts` send the user's current selection as a single overwrite — the server diffs against the DB, no client-side baseline-then-diff. `workspaceName` MUST come from the DB on resume: the plan step reads it from form state and passes it into `/checkout` as `workspaceName=...`, which becomes the Stripe subscription's `workspaceName` metadata. Without it the wizard's `"${user.name}'s workspace"` fallback lands in Stripe for any user who completes payment after a resume.
...
- Creating a workspace with `isFirstProject: true` (via `POST /api/workspaces`) already auto-creates the **Failure Detector** signal, its trigger, and the creator's email targets on default reports. The Signals step's PUT carries the user's full template selection; the server reconciles against existing template-named signals — so revisiting the step never duplicates Failure Detector or any previously-created template.
...
- Save handlers in `use-onboarding-actions.ts` must return `false` with a destructive toast on any failure (`!res.ok` or network error) — never advance optimistically. `track(...)` and `persistOnboardingStep(...)` sit after the save work and must also be gated on success.
  ...
  - `saveNotifications` PUTs `/api/workspaces/:id/reports` with `{ subscribedReportIds }`. The server-side `setEmailSubscriptions` action (in `frontend/lib/actions/reports/index.ts`) drops the session user's existing EMAIL targets for that workspace and re-inserts rows for every id in the payload — single transaction, no client-side diff, no baseline GET.
