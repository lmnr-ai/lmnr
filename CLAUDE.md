...
- `MDHeading` (`components/blog/md-heading.tsx`) must flatten `props.children` to plain text via a recursive walker before calling `headingToUrl` — `String(children)` yields `"[object Object]"` for headings containing inline markup (`**bold**`, `` `code` ``, links), producing an anchor that doesn't match the id `parseHeadings` derives from the raw markdown. Mismatched ids silently break TOC scroll-spy and permalink anchors.
