...
- Within one flush, `build_dedup_batch` deduplicates `(trace_id, hash)` pairs via a `HashSet` so two spans in the same trace referencing the same new message emit one `CHLlmMessage`, not two.
