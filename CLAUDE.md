...
- Server-side captures from NextAuth callbacks (e.g. `auth:user_created` in `lib/auth.ts`) must `await client.shutdown(timeoutMs)` after `client.capture(...)` — `posthog-node` batches by default and the serverless request can terminate before the batch flushes, dropping the event. Always pass an explicit timeout (we use 2000ms); `shutdown()` defaults to **30 seconds**, which would stall the login response for the full 30s if PostHog ingest is unreachable. Wrap the whole block in try/catch so analytics failures cannot break login.
...
- PostHog `distinctId` across the codebase is the user's **email**, not the DB user id. Client-side `identify(email)` in `provider.tsx` and server-side `posthog.identify({ distinctId: user.email })` in both workspace/project layouts all key on email. Any new server-side `client.capture(...)` (e.g. from NextAuth callbacks) must also pass `distinctId: email` — passing the DB user id creates a separate PostHog person profile and detaches `$set_once` properties and funnel events from the real user.
- Workspace grouping: `WorkspaceGroupTracker` (`components/common/workspace-group-tracker.tsx`) is rendered inside both `app/workspace/[workspaceId]/layout.tsx` and `app/project/[projectId]/layout.tsx` to attach the PostHog `workspace` group to every event. Group key is `workspace.name` (readable in the PostHog UI) with `{ name, id }` as traits. Do NOT key by id — PostHog group filters display the key directly. The `useEffect` is gated on `[workspaceName, workspaceId]` so it only re-fires on workspace switch, not every render.
- **React effect ordering caveat**: child effects fire before parent effects, so `WorkspaceGroupTracker` (and any other descendant of `PostHogProvider`) calls `group`/`identify`/`track` *before* `PostHogProvider`'s init effect runs. `lib/posthog/client.ts` handles this with a three-state flag (`pending` → `initialized` | `disabled`) and a `pendingCalls` queue: calls made while `pending` are buffered and flushed once `init()` succeeds, or discarded if telemetry is disabled. Don't revert to a simple `initialized` boolean — it silently drops first-paint events.
- **PostHogProvider must always render** in `app/layout.tsx`, with `telemetryEnabled={posthogEnabled}` controlling the behavior. Conditionally rendering it (e.g. `{posthogEnabled ? <Provider/> : body}`) means `init()` is never called on disabled installs, so the `status` flag stays `pending` forever and the `pendingCalls` queue grows unbounded on every descendant tracking call. Keeping the provider mounted ensures `init(false)` runs and flips status to `disabled`, turning subsequent calls into immediate no-ops.
