...
- The `spans_v0` view body mirrors the pre-PR prod shape exactly (bare column references, `multiIf` for `span_type`/`status`, no table alias) — the dict reconstruction slot replaces the bare `input` projection. Keep this shape when editing the view: the prod deployment has validated it at scale, and any deviation (switching `multiIf` back to `CASE WHEN`, introducing `s.` table aliases) can interact with ClickHouse's prewhere mover to produce `AMBIGUOUS_COLUMN_NAME: span_type String / span_type UInt8` under `WHERE span_type='LLM' ORDER BY start_time DESC LIMIT N` — an upstream CH 25.x analyzer issue unrelated to this PR.
