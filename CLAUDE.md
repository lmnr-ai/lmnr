...
- `pushItemsToDataset` only pushes rows where `is_labelled = true` and returns `{ pushed: <count> }` with a **200 status** even when nothing was pushed (e.g. caller passed an unlabelled `itemIds`). Any UI that calls `/push-to-dataset` must (a) disable the action for unlabelled items and (b) gate local removal / success toasts on `result.pushed > 0` — `res.ok` alone is not sufficient.
...
- The target auto-save in `frontend/components/queue/queue.tsx` tracks timers **per-item-id** in `saveTimersRef` / `abortByItemIdRef` / `lastSavedKeyRef` / `scheduledKeyRef` Maps. Do NOT collapse this back into a single `useDebounce(currentItem)`: a shared timer gets cleared when the user navigates within the 600 ms window, silently dropping A's edit if they tab to B. The scheduler effect re-arms a timer for an id ONLY when that id's target content changed since it was last scheduled — editing B must not defer A's pending save. On unmount, all still-pending timers synchronously call `flushSave`.
- Approve and discard must call `cancelPendingSaveFor(currentItem.id, target)` before their PATCH/DELETE: it (a) clears that id's timer + `scheduledKey`, (b) aborts any in-flight fetch, and (c) pre-sets `lastSavedKey` to the content we're about to send so the next render doesn't reschedule a duplicate save. Without this, a delayed `isLabelled:false` PATCH can arrive after approve's `isLabelled:true` PATCH — RMT resolves by `updated_at`, so last-write-wins silently reverts the approval. Same reasoning applies to discard: a late re-insert with a fresh `updated_at` resurrects a row past its delete tombstone.
- `globalTargetSelections` in `queue-store.tsx` is a sticky carry-over of field values the user has selected in *any* item during the session. It is applied by `applyGlobalsToIndex` on `setCurrentIndex` / `step` navigation — NOT on `setItems` (which would fight SWR refreshes). Merge rule: item's own `payload.target` keys win over globals; labelled items are skipped; the item is marked dirty so the debounced save persists the carried-over values.
- `getQueueItems` in `frontend/lib/clickhouse/labeling-queue-items.ts` treats `limit`/`offset` as *opt-in*. When called with no options (as `listQueueItems` → the queue items route does) it emits a query with no `LIMIT` clause and returns every row. Do NOT restore a default cap here — `queue.tsx` loads the full list into Zustand for in-browser navigation and an implicit 1000-row default silently truncated large queues. If you need pagination later, add it at the consumer layer, not here. The UI's `progress.total` reads `data?.progress.total` (from `getQueueCounts`, also FINAL) as the authoritative count and only falls back to `items.length` before the first fetch.
