...
- `Outbox<T>` (`app-server/src/mq/outbox.rs`) is the generic primitive for **fire-and-forget, drop-on-overflow** publishes from a hot path. Each instance is bound at construction to one (exchange, routing_key) destination via `OutboxConfig { name, exchange, routing_key, default_capacity, default_shipper_concurrency }`. The `name` doubles as the env-var prefix: with `name = "indexer"`, runtime overrides are `INDEXER_OUTBOX_CAPACITY` / `INDEXER_OUTBOX_SHIPPERS`. Internals: bounded `mpsc::channel` of `Vec<u8>` (serialization happens once at `try_send`, not in the shipper) + a dispatcher task that fans each item out to a `Semaphore`-bounded `tokio::spawn` calling `MessageQueue::publish`. When all shipper slots are busy and the channel fills, `try_send` drops the payload (counter increments, log at every power-of-two). Use this only for payloads that are **reproducible / non-essential** — never for primary writes that must not be lost.
- The Quickwit indexer publish in `process_span_messages` goes through `IndexerOutbox` (a `pub type IndexerOutbox = Outbox<IndexerQueuePayload>` alias in `app-server/src/quickwit/outbox.rs`) — instantiated via `spawn_indexer_outbox(queue)` with defaults `capacity=2048, concurrency=4`. The outbox is spawned once at consumer-runtime boot in `main.rs` and shared by `SpanHandler` + `DataPlaneSpanHandler`. Do NOT add a new caller to `process_span_messages` without threading `Arc<IndexerOutbox>` in alongside `queue`. New outbox use cases (e.g. signals queue in the private fork) should add their own `pub type` + spawn helper instead of reusing `IndexerOutbox` — each destination wants its own capacity/concurrency budget and its own log-line name.
