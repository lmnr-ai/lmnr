...
- The Signals sidebar item is behind a feature flag (`Feature.SIGNALS`) which requires `SIGNALS_ENABLED=true` AND a configured AI provider (`LLM_PROVIDER` + `LLM_API_KEY` + `LLM_MODEL_SMALL|MEDIUM|LARGE`, or AWS Bedrock credentials). See "AI Features Model Setup" above.
