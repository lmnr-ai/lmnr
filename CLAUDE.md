...
- Ingest order in `process_span_messages`: build dedup batch synchronously, then run the trace upsert, the `llm_messages` insert (+ `mark_seen` on success / `unmark_seen` on failure), and the `spans` insert concurrently via `tokio::join!` (LAM-1609). The strict `llm_messages → mark_seen` ordering still holds inside the messages branch — never flip it. Within one flush, `build_dedup_batch` dedupes `(project_id, trace_id, hash)` triples via HashSet — the key MUST match `llm_messages` ORDER BY since a batch can mix projects.
- Parallelizing `llm_messages` and `spans` accepts a brief inconsistency window: a span row can land in CH before its referenced messages do. Reconstruction via `llm_messages_dict` returns `'null'` for missing hashes (`dictGetOrDefault`), so readers see partial input for ms-scale until both inserts complete. If `llm_messages` fails but `spans` succeeds, the batch still returns `HandlerError::transient` (so it retries), Redis is `unmark_seen`'d, and the messages re-insert lands on retry — `spans` is RMT-keyed by `span_id` so the re-insert collapses cleanly. Net effect: orphaned-hash spans exist only between the failed insert and the next retry tick.
