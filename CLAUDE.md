...
- Producer NEVER writes Redis. `mark_seen` / `unmark_seen` still run only on the consumer, AFTER a successful `llm_messages` insert. The "stamp only after success" invariant is preserved exactly — the producer's Redis check is read-only and best-effort (errors fall through to "treat as new", same as the consumer's prior behaviour).
