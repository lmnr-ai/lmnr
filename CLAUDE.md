...
- Resume hydration (`lib/actions/onboarding/hydrate.ts`) re-derives form defaults (`workspaceName`, `selectedSignalIds`, `emailNotificationsEnabled`, `slackConnected`, `selectedTier`) from the DB on every page load when a workspace/project are bound, so refreshing or revisiting steps shows the user's actual current state. Save handlers in `use-onboarding-actions.ts` therefore reconcile the diff (create + delete) against existing resources rather than blindly creating new ones — `saveSignals` deletes signals matching template names that the user unchecked, `saveNotifications` flips email subscriptions per-report based on `emailOn`. `workspaceName` MUST be hydrated on resume: the plan step reads it from form state and passes it into `/checkout` as `workspaceName=...`, which becomes the Stripe subscription's `workspaceName` metadata. Without hydration the wizard's `"${user.name}'s workspace"` fallback lands in Stripe for any user who completes payment after a resume.
...
- Creating a workspace with `isFirstProject: true` (via `POST /api/workspaces`) already auto-creates the **Failure Detector** signal, its trigger, and an email alert target for the creator. The Signals step's reconcile reads existing signals from `GET /api/projects/[projectId]/signals` and only POSTs ones that aren't already there — so revisiting the step never duplicates Failure Detector or any previously-created template.
...
- Plan step: "free" is the default. Selecting a paid tier and clicking Continue does `window.location.href = /checkout?lookupKey=...`. `app/checkout/page.tsx` hardcodes Stripe's `success_url` / `cancel_url` to `/workspace/${workspaceId}?tab=billing`, so the user does NOT return to `/onboarding` — the paid-tier branch must therefore mark onboarding complete before redirecting (track `onboarding:completed` + `await fetch('/api/onboarding/state', { method: 'DELETE' })`). Do NOT use `keepalive` fire-and-forget here: `keepalive` keeps the request in flight across navigation but the response's `Set-Cookie` only updates the cookie jar while the current document is alive, so the cookie can survive, and the `(authenticated)` layout on the billing page would bounce the paid user back to `/onboarding`. Non-subscription environments (`Feature.SUBSCRIPTION` off, read via `useFeatureFlags()`) short-circuit to a single "Finish" button.
...
- **Every onboarding-completion DELETE of `/api/onboarding/state` must gate the next navigation on `res.ok`, not just on the fetch promise resolving.** `fetch` only throws on network errors — any HTTP status (401 on session expiry, 500 from the route handler) resolves normally. Navigating into the `(authenticated)` tree with a live cookie loops via the layout gate → `/onboarding`. This applies to `clearOnboardingStateCookie` in `components/onboarding/use-onboarding-actions.ts` (returns boolean; `finishFreeTier` propagates it and toasts on failure) and the inline DELETE in `components/onboarding/steps/plan-step/index.tsx` (paid-tier branch reads `res.ok`, toasts + `endSubmitting()` on failure, only calls `window.location.href = checkoutUrl` on success).
- The wizard's mount-POST to `/api/onboarding/state` (which marks the user as in-progress) lives inside `OnboardingProvider` (`components/onboarding/context.tsx`), not in the wizard component. The provider exposes `waitForMountPersist()` which returns a promise that resolves when the POST settles. `finishFreeTier` and the paid-tier branch of `plan-step/index.tsx` MUST `await waitForMountPersist()` before their DELETE — on a resume at step 4 the user can click Finish before the POST lands, and the POST's Set-Cookie would otherwise arrive AFTER the DELETE, resurrecting the cookie and bouncing the user back to `/onboarding` via the `(authenticated)` layout gate. The promise resolves on both success and failure (it's best-effort), so this only costs the pending-RTT and never blocks on a dead POST. Do NOT re-introduce the mount POST in `components/onboarding/index.tsx`'s `useEffect` — its promise would be unreachable from the completion handlers.
- Reconcile save handlers in `use-onboarding-actions.ts` fetch a baseline from the server before diffing create/delete. If that prerequisite GET fails (`!res.ok` or network error), return `false` with a destructive toast — do NOT fall back to `existing = []` or advance. `saveSignals`'s GET is `/api/projects/:id/signals`: an empty fallback would re-POST the auto-created Failure Detector (plus any previously-saved templates on resume), producing duplicates or 4xx errors. `saveNotifications`'s GET is `/api/workspaces/:id/reports`: returning `true` on failure would let the UI advance past notifications, `recordSlackStep` would then write step 4, and a later resume would silently skip notifications. Callers (`signals-step.tsx`, `notifications-step.tsx`) only advance on `ok`, so returning `false` keeps the user on the step to retry. `track(...)` and `persistOnboardingStep(...)` sit after the reconcile work and must also be gated on the GET having succeeded.
...
- Tracking calls must be guarded by success (`res.ok`). Firing `track(...)` after an unchecked `await res.text()` records events for failed requests and corrupts metrics. This applies to *retryable* flows too — e.g. onboarding's "completed" event fires from inside `finishFreeTier` (and the paid-tier branch of plan-step) only AFTER the cookie-clearing DELETE returns 2xx, so that a toasted retry doesn't double-count the event on every click.
