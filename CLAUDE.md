...
- Signal events get their own Quickwit index `signal_events` (config in `frontend/lib/quickwit/indexes/signal_events.yaml`, applied at frontend boot by `initializeQuickwitIndexes`). Default search fields are `summary`, `payload`, `name` — `summary` is the LLM-generated paragraph the user sees in the events table, `payload` is the JSON the signal emitted, `name` is the signal/event label. Structural ids (`project_id`, `signal_id`, `trace_id`, `run_id`, `id`) are tokenizer:raw + fast for filter joins; `severity` is `u64` (Quickwit doesn't accept `u8`).
- The producer hook is `publish_signal_events_for_indexing(events: &[CHSignalEvent], queue)` in `app-server/src/quickwit/producer.rs`. It mirrors the span/event indexer pipeline (`IndexerQueuePayload::SignalEvents` variant in `quickwit/mod.rs`, routed by `QuickwitIndexerHandler` in `consumer.rs`). The actual call site lives in `lmnr-private`'s signals handler — it runs alongside `push_to_event_clustering_queue` after `signal_events::insert_signal_events` succeeds. Errors must be logged + swallowed at the call site (matches the span/event indexer posture); a Quickwit hiccup never blocks signal-event creation.
- `QuickwitIndexedSignalEvent::flatten_json` parses `payload` as JSON and walks it via `extract_text_from_json_value` to make nested keys tokenizable; if the parse fails, the raw string falls through and remains searchable. `preprocess_for_indexing` runs `preprocess_text` on `name` / `summary` / `payload` (ANSI strip + whitespace collapse + NFC) — same path as `QuickwitIndexedEvent`. Cleaning happens in the consumer, not at `from(&CHSignalEvent)`, so the producer call stays cheap.
- `CHSignalEvent.timestamp` is stored as `i64` nanoseconds-since-epoch (CH `DateTime64(9, 'UTC')`). The `From<&CHSignalEvent>` impl on `QuickwitIndexedSignalEvent` reconstructs `DateTime<Utc>` via `from_timestamp(secs, nanos)`. The Quickwit `timestamp` field accepts the resulting RFC3339 serialisation natively — no manual formatting.
- Search route: `POST /api/v1/projects/{projectId}/signal-events/search` (`app-server/src/routes/signal_events.rs`) wraps `crate::search::signal_events::search_signal_events`. It returns `{ ids: string[] }` only — the frontend then hydrates rows from CH so the existing column projection / pagination / cluster-filter / payload-field columns stay unchanged. The events route at `/api/projects/[projectId]/signals/[id]/events` accepts a `searchQuery` query param; when set, `getEventsPaginated` calls the search endpoint first, short-circuits to `{items: [], count: 0}` on no hits (avoids emitting `id IN ()` which is a CH syntax error), and otherwise threads the id list as a CH `id IN {ids:Array(UUID)}` custom condition via the new `idFilter` option on `BuildEventsQueryOptions`.
- The query Quickwit receives is `project_id:{pid} AND signal_id:{sid} AND ({escaped_query})` against `default_search_fields = [summary, payload, name]`. `escape_quickwit_query` is shared with the spans search via `pub(crate)` re-export from `app-server/src/search/mod.rs` — don't fork the escape table.
