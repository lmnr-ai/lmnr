...
- Idempotency is **best-effort**, not a hard guarantee. RMT has no unique constraints. The Rust `create_labeling_queues_items` handler does a FINAL-scoped `idempotency_key_exists` lookup before each insert and additionally dedupes keys appearing more than once within the same batch via a `seen_keys` HashSet (the existence check only sees rows already committed to CH). Two concurrent inserts with the same key inside the pre-check window can still produce duplicate rows; acceptable because real-world retries are timed in seconds. Item ids are always `Uuid::now_v7()` — there is no deterministic-id collapse trick.
