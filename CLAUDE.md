...
- One `alertTargets` row = one `(alertId, SLACK, channelId)` pairing. The Rust notifier in `app-server/src/notifications/` already iterates every SLACK target per alert and posts separately, so multi-channel alerting is a pure frontend concern — do not introduce backend fan-out logic for Slack.
- `channelName` (not `channelId`) is the durable identifier when editing an existing alert. Slack channel IDs can rotate (e.g. when a channel is archived/recreated or a bot is re-invited), but the human-readable name is what the user typed. Rehydrate the tag input from `alert.targets[].channelName` and re-resolve to an ID via `verifySlackChannels` on save.
- `verifySlackChannels` (`frontend/lib/actions/slack/index.ts`) pages through `conversations.list` (up to `MAX_VERIFY_PAGES=50` × `VERIFY_PAGE_SIZE=1000`) because Slack exposes no search API and per-name `conversations.info` requires IDs. Pagination short-circuits as soon as every requested name is resolved, so typical workspaces finish in one page. Do NOT reintroduce a client-side `conversations.list` dropdown — workspaces with >200 channels break it (LAM-1507).
- When `hasSlackIntegration` is false but the alert already has SLACK targets (integration was disconnected after-the-fact), `onSubmit` in `manage-alert-sheet.tsx` preserves those existing targets verbatim. Removing this branch would silently wipe Slack delivery any time an admin edits an alert without the integration reconnected.
- Legacy SLACK `alertTargets` rows may have a `channelId` but a null/empty `channelName` (pre-LAM-1507 alerts or earlier migrations). The tag-input rehydration filters by `channelName`, so those rows can't appear in the UI. `onSubmit` in `manage-alert-sheet.tsx` must also carry them over into the new `targets[]` when `hasSlackIntegration` is true (edit mode), otherwise `updateAlert`'s delete-then-reinsert wipes them and the user silently loses Slack delivery on that channel.
