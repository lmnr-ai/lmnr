...
- AI SDK v7 (`ai.prompt.messages`) traces fall through every provider detector and land on the **generic** renderer. Backend `from_instrumentation_content_part` re-serialises tool calls as `{type: "tool_call", name, id, arguments}` (snake_case via serde rename_all) and tool results as `{type: "tool-result", toolCallId, toolName, output}` (kebab-case via explicit `serde rename`). `processContentPart` in `frontend/lib/spans/types/index.ts` therefore needs an explicit `tool-result` case — the `default` branch's tool-role fallback would `JSON.stringify(part)` the entire envelope into `output.value`, surfacing the part metadata inside the rendered output. Tool calls already work via `case "tool_call"`. The `tool-result` case must NOT re-wrap `output`: the AI SDK's `LanguageModelV2ToolResultOutput` is already a tagged envelope (`{type: "text"|"json"|"error-text"|"error-json"|"content", value}`) which the backend's `serde_json::Value` preserves verbatim, so wrapping it again as `{type: "text", value: JSON.stringify(envelope)}` would surface the stringified envelope after the renderer's single-level unwrap. Pass envelopes through verbatim; only wrap bare values (string → `{type: "text"}`, anything else → `{type: "json"}`). Envelope detection lives in `isAISDKToolResultEnvelope` (exported from `frontend/lib/spans/types/index.ts`) and **validates `type` against the known literals** — checking `"type" in output && "value" in output` alone falsely matches user payloads like `{type: "user", value: 100, name: "Jane"}` and silently drops sibling fields when the renderer unwraps to `value`. The generic `GenericToolResultContentPart` (`frontend/components/traces/span-view/generic-parts.tsx`) reuses the same helper and unwraps `output.value` only for verified envelopes; `GenericToolCallContentPart` passes only `part.input` (not `omit(part, "type")`) so the rendered JSON is just the args, not redundant `toolCallId/toolName/input` envelope.
