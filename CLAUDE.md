...
- Alert Slack targets persist a channel `id` in `alertTargets.channelId` and the display name in `alertTargets.channelName`. Pre-LAM-1507 rows may have stored the channel *name* in `channelId` instead of an ID, so consumers must tolerate both shapes. `manage-alert-sheet.tsx`'s `resetFormFromSignals` falls back `channelId ?? channelName` on load, then upgrades name-only entries to real IDs once the live channel list resolves (via name lookup).
- `getSlackChannels` in `frontend/lib/actions/slack/index.ts` paginates through the full Slack `conversations.list` result via `response_metadata.next_cursor` (limit=1000/page) and retries on 429s (honoring `Retry-After`) plus 200 OK `{ok:false, error:"ratelimited"}` bodies. Do NOT cache — each request fetches fresh so new/renamed channels appear immediately. The picker (`slack-channel-picker.tsx`) virtualizes the result with `@tanstack/react-virtual`, so thousands of channels stay cheap.
