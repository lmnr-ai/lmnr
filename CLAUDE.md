...
- AI generation for render templates is a chat-shaped, multi-turn `generateObject` call (NOT streaming): route at `app/api/projects/[projectId]/render-templates/generate/route.ts` returns plain JSON, action `generateRenderTemplate(prompt, history, currentCode, testData)` at `lib/actions/render-template/generate.ts`, prompts at `lib/actions/render-template/prompts.ts`. The client hook (`manage-template-dialog/use-stream-template.ts` — file name is legacy; the export is `useTemplateChat`) maintains a per-dialog `messages` array, sends prior turns as `messages` to `generateObject`, and on each successful turn writes the model's `result` to the form's `code` field via `onTurnComplete`. The schema's `summary` field is what we render in the chat bubble (one-line description of the change); `result` is the full JSX the iframe will render. We deliberately do NOT stream partial JSX because half-written JSX makes babel throw compile errors in the live iframe. While a turn is in flight the preview slot conditionally swaps the iframe for a `StreamingPreview` sibling that renders a static skeleton wireframe (file name is also legacy). Chat history is dialog-session only: closing the dialog calls `resetAi()` and the next open starts fresh from the saved template's `code`. The hook stores the assistant's full `result` (not the `summary`) in `persistedHistoryRef` so the next turn's history payload reflects exactly what the model produced. Earlier iterations tried an absolute-positioned overlay on the iframe and a streaming setup with `streamObject` + `toTextStreamResponse()` + a `plan` field — both dropped. The overlay required `backdrop-blur` + `bg-background/85` to mask stale contents; streaming carried complexity (partial-JSON parsing, abort plumbing) for no perceptible benefit since neither preview nor code-editor renders partials.
