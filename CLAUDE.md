...
- `proxy.ts` is now scoped to its original responsibilities only: API authz checks (`/api/projects/*`, `/api/workspaces/*`, `/api/shared/traces/*`) and the Strapi rewrite for `/uploads/*`. It does not import anything from the onboarding feature.
