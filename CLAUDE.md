...
- `AdvancedSearchStoreProvider` creates the Zustand store once via `useState(() => createAdvancedSearchStore(...))`, but the `filters` prop is synced into the store reactively via a `useEffect` that calls `setFilters`. Consumers with **dynamically-loaded** filter definitions (e.g. evaluation datapoints where score columns are fetched async and appended to `columnFilters`) rely on this — without it, `addTag` / `addCompleteTag` / `updateTagField` would run `filters.find()` against the stale mount-time list and silently fail for the newly-loaded columns. The effect uses `isEqual` to avoid spurious writes when the prop array identity changes but content is the same.
