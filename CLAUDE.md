...
- "Open in SQL editor" is a shared hook (`useOpenInSql` in `frontend/components/traces/trace-view/use-open-in-sql.tsx`) with a discriminated `Params` union — `span | trace | signal`. To add a new entity type, extend the union and add a `buildQuery` case; callers keep the same `{ openInSql, isLoading }` return shape. The hook POSTs to `/api/projects/{projectId}/sql/templates` and opens `/project/{projectId}/sql/{templateId}` in a new tab.
- `ManageSignalContent` uses `showTemplates = !id` as the create-vs-edit discriminator. UI gated to the edit form (e.g. Copy prompt / Copy schema buttons in `signal-form-fields.tsx`) should key off `!showTemplates`, not a separate prop. The template picker only renders in create mode.
