...
- Redis (`m:{project_id_simple}{hex(hash)}`, TTL 1h) is ONLY a hot-path "seen recently" filter that skips re-emitting identical rows on every agent step. It is best-effort: on `cache.exists()` error we always insert and rely on ReplacingMergeTree to collapse duplicates on merge. The `mark_seen` call happens only AFTER a successful `llm_messages` insert; on insert failure `unmark_seen` is called and the whole batch returns `HandlerError::transient` so it is retried. Never flip the order — stamping Redis before the insert can leave a permanent "seen" marker for a row that never landed.
