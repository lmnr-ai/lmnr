...
- Transcript view (not Tree) is the intended default for these captures. `TraceViewStore` v1 migrate bumps legacy `tree` persisted tab to `transcript`, so fresh browser sessions default correctly — but persisted localStorage (`trace-view-state`) overrides can pin the view to Tree. If a capture run shows tree, clear the store key or select Transcript via the view dropdown before shooting.
- Navigating between trace drawers in a single session can leave stale drawer content (previous trace's span input/output still rendered after URL change). Reloading with `ignoreCache: true` forces the drawer to re-fetch. After any `navigate_page` to a new `traceId`, verify via `evaluate_script` that body text contains expected markers (e.g. new trace's span name) before taking the screenshot.
- Chrome MCP `take_screenshot` can return the pre-resize viewport after a fresh `navigate_page` — call `resize_page(1512, 982)` first, wait ~3s, then screenshot.
- Generating local traces in the sandbox: the default `LMNR_PROJECT_API_KEY` in `/sandbox/scratch/.env` may point at a project that is NOT registered with the local app-server (ingestion fails as UNAUTHENTICATED). Create/pull a project API key from the running local app-server's DB and pass it explicitly to `Laminar.initialize(project_api_key=...)` in verification scripts rather than relying on the env default. Do NOT commit the key into CLAUDE.md or any tracked file.
