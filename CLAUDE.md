...
- The Quickwit indexer publish in `process_span_messages` does NOT call the broker on the consumer hot path. Payloads go to `IndexerOutbox` (`app-server/src/quickwit/outbox.rs`) — a bounded `mpsc::channel` with capacity `QUICKWIT_OUTBOX_CAPACITY` (default 2048) and a dispatcher task that fans each item out to a `Semaphore`-bounded `tokio::spawn` (concurrency `QUICKWIT_OUTBOX_SHIPPERS`, default 4). Each spawn calls `MessageQueue::publish` and drops on error. When all shipper slots are busy and the channel fills, upstream `try_send` returns `Full` and the payload is dropped (counter increments, log at every power-of-two). This is the explicit back-pressure boundary: indexer payloads are reproducible from ClickHouse, so drop-on-overflow is preferable to blocking the consumer pipeline behind a stuck broker. The outbox is spawned once at consumer-runtime boot in `main.rs` and shared by `SpanHandler` + `DataPlaneSpanHandler`. Do NOT add a new caller to `process_span_messages` without threading `Arc<IndexerOutbox>` in alongside `queue`.
