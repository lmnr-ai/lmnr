...
- Event clusters live in two ClickHouse tables: `signal_event_clusters` (one row per cluster with `name`, `level`, `centroid Array(BFloat16) CHECK length = 3072`, `parent_id`, `num_signal_events`) and `events_to_clusters` (event-id → cluster-id mapping). Frontend queries use the bare names `clusters` and `signal_events`, which the query-engine rewrites to `clusters_v0` / `signal_events_all_v0` views that apply the `project_id = {project_id:UUID}` filter. The `clusters_v0` view also hides L0 clusters (`WHERE level > 0`), so seeded data must use `level = 1+` to appear in the UI.
- Both `signal_event_clusters` and `events_to_clusters` use `SharedReplacingMergeTree`. To fix a stale row, insert a replacement with the same key and a newer `updated_at`; run `OPTIMIZE TABLE ... FINAL` to collapse immediately. Seeded centroids can be `arrayResize([toFloat32(0)], 3072, toFloat32(0))` (zero vector is accepted by the `length = 3072` CHECK constraint). Set `parent_id` to the all-zero UUID for root-level clusters — the frontend filters that value back to `null` in `getEventClusters`.
