...
- Idempotency is **best-effort**, not a hard guarantee. RMT has no unique constraints. The Rust `create_labeling_queues_items` handler does **one** FINAL-scoped `existing_idempotency_keys` lookup per request (the batch's non-empty keys are passed as `IN { keys: Array(String) }`, with `DISTINCT idempotency_key` to flatten same-key-different-id duplicates that FINAL keeps separate) and additionally dedupes keys appearing more than once within the same batch via a `seen_keys` HashSet. Committed state wins over batch-local — an item whose key is in `already_present` is dropped before `seen_keys` even sees it, matching the old Postgres `ON CONFLICT DO NOTHING` precedence. Two concurrent inserts with the same key inside the pre-check window can still produce duplicate rows; acceptable because real-world retries are timed in seconds. Item ids are always `Uuid::now_v7()` — there is no deterministic-id collapse trick (it would silently rewrite `payload` on retry under RMT's last-write-wins-by-`updated_at`, breaking the payload-is-immutable invariant).
...
- `pushItemsToDataset` and `removeQueueItem` (in the `datasetId` branch) BOTH compute the datapoint `target` via the shared `effectiveTarget(item)` helper — which under the mirror model just parses `edit`. The DELETE route no longer accepts `data`/`target`/`metadata` in the request body; the server fetches the row itself (`getQueueItems(..., { ids: [id] })`) so the resolution rule can't drift between client and server. Both paths do `createDatapoints` (with fresh UUIDv7 ids) then `deleteQueueItems` as two separate ClickHouse writes with no transaction. The single invariant is **create-before-delete** so a failed insert leaves the queue item intact for retry. Datapoint ids are **non-deterministic**, and `dataset_datapoints` is plain `MergeTree` (not `ReplacingMergeTree`) so a retry after a partial failure (insert succeeded, delete failed) WILL produce duplicate dataset rows. The push paths are gated by the UI (button disabled while in-flight), so retries are an explicit user action and the duplication is treated as an acceptable trade-off for not maintaining a deterministic-id + existence-check layer.
