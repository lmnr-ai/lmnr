...
- `StaleResumeRedirect` (`components/onboarding/stale-resume-redirect.tsx`) is the client-bridge used when the resume cookie points at a workspace/project that was deleted out from under the user. It MUST gate `router.replace(destination)` on the DELETE's `res.ok`, not just on the fetch promise resolving — `fetch` resolves on any HTTP status and only throws on network errors, so a 401/500 from `/api/onboarding/state` would leave the cookie live. Navigating into the `(authenticated)` tree with a live cookie triggers an infinite loop via the layout gate → `/onboarding` → stale-resume → redirect. On DELETE failure, render a retry affordance instead of navigating.
