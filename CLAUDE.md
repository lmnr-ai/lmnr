...
- Install-level dedup across replicas uses a single-row Postgres UPDATE with a cooldown interval (`UPDATE self_hosted_instance SET last_event_at = NOW() WHERE last_event_at IS NULL OR last_event_at < NOW() - '1 hour'::interval RETURNING id`). The row is seeded by a migration, never inserted from app code — a missing row fails silently and skips the event. Launch uses 1h cooldown (covers rolling deploys), heartbeat uses 23h (daily-ish without clock-skew drift, ticks hourly).
