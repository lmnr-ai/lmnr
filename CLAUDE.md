...
- `pushItemsToDataset` does `createDatapoints` then `deleteQueueItems` as two separate ClickHouse writes with no transaction. To keep the transfer retry-safe, the inserted datapoint `id` is **deterministic** per queue item: UUIDv5 over `(datasetId, queueItemId)` via `datapointIdForQueueItem` in `frontend/lib/utils.ts`, and `filterExistingDatapointIds` in `frontend/lib/clickhouse/datapoints.ts` skips rows that are already present. This matters because `dataset_datapoints` is a plain `MergeTree` (not `ReplacingMergeTree`), so a retry with random ids would leave permanent duplicate rows in the dataset.
