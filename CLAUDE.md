...
- Ingest order in `process_span_messages` (LAM-1609): build dedup batch synchronously, then run two branches concurrently via `tokio::join!` — (a) the trace upsert (PG `traces` → CH `traces` → realtime dispatch) and (b) the strict span path `llm_messages → mark_seen → spans`. The trace branch is independent of the span path so it parallelizes safely; **the span path stays sequential** because `spans` is plain MergeTree (NOT RMT) — if `spans` inserted before `llm_messages` and `llm_messages` failed, the batch retry would re-insert the same `CHSpan`s and CH would store every span twice. Within one flush, `build_dedup_batch` dedupes `(project_id, trace_id, hash)` triples via HashSet — the key MUST match `llm_messages` ORDER BY since a batch can mix projects. Do NOT collapse to a 3-way join across `llm_messages` and `spans`.
