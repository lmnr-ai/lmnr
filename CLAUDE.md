...
- `RabbitMqSpanMessage` carries two new fields with serde defaults: `pre_processed: bool` (consumer skips `parse_and_enrich_attributes` + `convert_span_to_provider_format` when true) and `input_dedup: Option<LlmInputDedupPlan>` (consumer dedup batch path). Old agents that emit messages without these fields fall through to the legacy on-consumer path (`build_dedup_batch_legacy`) byte-identically.
...
- Plan-based vs legacy branch in `process_span_messages`: if ANY recordable span in the flush has `Some(plan)`, the entire batch goes through `build_dedup_batch_from_plans`; spans without a plan in that batch contribute empty hashes (same as non-LLM / non-array input fall-through). Only when EVERY span lacks a plan do we hit `build_dedup_batch_legacy` (which still hashes + checks Redis inline). This means a mixed flush of legacy + producer-preprocessed agents works correctly — the legacy spans just don't get producer-side savings.
- Quickwit indexing reads pre-built `dedup.span_new_message_values` (a `Vec<Vec<Value>>` aligned with the spans batch) instead of projecting indices back onto `span.input` — required because `span.input` is `None` on the producer path. Both `build_dedup_batch_from_plans` and `build_dedup_batch_legacy` populate this field so the indexer code path is uniform.
