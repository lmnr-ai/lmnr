...
- `saveNotifications` in `use-onboarding-actions.ts` performs a prerequisite GET on `/api/workspaces/:id/reports` before reconciling subscriptions. `track("notifications_configured")` and `persistOnboardingStep(..., 3)` must only fire when that GET was 2xx — the `!res.ok` and `catch` branches both early-return `false` with a toast so the caller (`notifications-step.tsx`, which only advances on `ok`) keeps the user on the notifications step to retry. Returning `true` on failure desyncs the wizard: the UI advances, `recordSlackStep` writes step 4, and a later resume lands on step 4 with notifications silently skipped.
