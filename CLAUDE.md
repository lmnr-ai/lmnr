...
- Signal trigger columns map to ClickHouse `traces` view as follows (see `frontend/lib/actions/signals/estimate.ts` and the Rust `Trace::evaluate_single_filter`): `span_name` → `has(span_names, X)` (span_names is an Array aggregated across batches), `status` → `status` column (String), `total_token_count` → `total_tokens` (Int64), `root_span_finished` → `top_span_id != toUUID('00000000-...')` regardless of operator/value (Rust only checks `top_span_id.is_some()`). When adding new filter columns, keep the Rust evaluator and the estimate SQL mapper in sync.
- Signal run estimation (`/api/projects/:id/signals/estimate`) translates triggers into ClickHouse count() queries over `trace_type = 'DEFAULT'` traces in the last 24h or 30d. Realtime triggers (`mode=1`) are billed as 2 runs per match (matching the UI copy in `triggers-section.tsx`); batch (`mode=0`) as 1. Returns HTTP 422 with `code: NOT_ENOUGH_DATA` when the oldest trace in the project is newer than the window — the estimate row shows an amber underlined "Not enough data" link with a tooltip rather than a generic error.
- The "oldest trace" probe must use `minOrNull(start_time)` (not `min`). Plain `min()` on an empty DateTime64 column returns the epoch (1970-01-01) rather than NULL, which would silently pass the "no traces" null-check and surface as "0 runs of 0 traces" instead of the NotEnoughData panel.
- The estimate UI (`frontend/components/signals/create-signal-drawer/estimate-section.tsx`) is a single inline row (label + info tooltip left, count/status + Day/Month tabs right), not a labeled card. When adding new estimate states, keep them as short inline text and reuse the underlined-dotted tooltip pattern for errors. No per-trigger breakdown — the v1 card was intentionally boiled down for visual density.
- Enabling the Signals feature in local dev requires `SIGNALS_ENABLED=true` in `frontend/.env.local` AND a `GOOGLE_GENERATIVE_AI_API_KEY` (or AWS Bedrock creds) — without `SIGNALS_ENABLED`, the Signals sidebar entry and feature routes are hidden. The frontend must be restarted to pick up the env var change.
