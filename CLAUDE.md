...
- `saveNotifications` in `use-onboarding-actions.ts` performs a prerequisite GET on `/api/workspaces/:id/reports` before reconciling subscriptions. `track("notifications_configured")` and `persistOnboardingStep(..., 3)` must only fire when that GET was 2xx — the `!res.ok` and `catch` branches both early-return with a toast. Otherwise a 500/403 corrupts the metric AND advances the server-side step cursor to 3 despite no reconcile having happened, so a later resume lands on step 4 with stale subscription state.
