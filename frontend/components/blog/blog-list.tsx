...
const ThumbImage = ({
  ...
  <div className={cn("relative overflow-hidden bg-surface-200 rounded shrink-0", className)} style={{ width, height }}>
    {src && (
      <Image
        src={src}
        alt={alt}
...
const ListRow = ({ post, routePrefix, isFirst }: { post: BlogListItem; routePrefix: string; isFirst: boolean }) => (
  <Link
    ...
    className={cn("flex items-start gap-10 py-5 no-underline group", !isFirst && "border-t border-surface-300")}
  >
    {post.data.image && (
