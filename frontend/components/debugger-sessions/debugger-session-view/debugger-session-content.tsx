...
export default function DebuggerSessionContent({ sessionId, spanId }: DebuggerSessionContentProps) {
  ...
  useRealtime({
    key: `debugger_session_${sessionId}`,
  ...
  if (isEmpty(spans) && !isSpansLoading && sessionStatus === "PENDING") {
    return (
      <div className="flex items-center justify-center p-6 h-full">
        <div className="flex flex-col items-center gap-4 p-6 rounded-lg border bg-card text-card-foreground">
          ...
          <p className="text-sm text-muted-foreground text-center max-w-sm">
            Run the debugger to start, or traces will appear here when your code runs.
          </p>
        </div>
      </div>
    );
