...
export const EvaluationCard = ({
  ...
  return (
    <div
      ...
      className="group scroll-mt-4 overflow-hidden rounded-lg border border-[rgba(232,232,232,0.1)] bg-surface-200"
    >
      ...
      {expanded && (
        <>
          ...
          {progression && progression.scores.length > 0 && (
            <div className="h-32 px-2 py-1 bg-surface-100 border-t">
              <CombinedChart
          ...
          {scores.length > 0 && (
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 border-t px-4 py-3 md:grid-cols-3 lg:grid-cols-4 bg-surface-200">
              {scores.map((score) => (
                <ScoreStat
                  key={score.name}
                  name={score.name}
