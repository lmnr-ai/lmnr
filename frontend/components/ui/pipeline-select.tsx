...
export default function PipelineSelect({
  ...
  const [pipelineVersions, _setPipelineVersions] = useState<
    PipelineVersion[] | null
  >(null);
  const { projectId } = useProjectContext();

  // TODO: Fix this hack
  const [defaultPipelineNameState, setDefaultPipelineName] = useState<
    string | undefined
  >(defaultPipelineName);
  ...
  const setPipelineVersions = (pipelineVersions: PipelineVersion[]) => {
    if (hideWorkshopVersions) {
      pipelineVersions = pipelineVersions.filter(
        (version) => version.pipelineType !== 'WORKSHOP'
      );
    }
  ...
  useEffect(() => {
    ...
    if (
      !!defaultPipelineVersionId &&
      (!defaultPipelineVersionName ||
        !defaultPipelineIdState ||
        !defaultPipelineName)
    ) {
      // Note: This is a heavy request (returns graph too), but ok for now
      fetch(
        `/api/projects/${projectId}/pipeline-versions/${defaultPipelineVersionId}`
      )
        .then((res) => res.json())
        .then((pipelineVersion) => {
          ...
          if (!defaultPipelineIdState) {
            fetch(
              `/api/projects/${projectId}/pipelines/${pipelineVersion.pipelineId}`
            )
              .then((res) => res.json())
              .then((pipeline) => {
                ...
                fetch(
                  `/api/projects/${projectId}/pipelines/${pipeline.id}/versions`
                )
                  .then((res) => res.json())
    ...
    if (!!defaultPipelineIdState) {
      ...
      fetch(
        `/api/projects/${projectId}/pipelines/${defaultPipelineIdState}/versions`
      )
        .then((res) => res.json())
        .then((pipelineVersions) => {
          setPipelineVersions(pipelineVersions);
    ...
    <div className="flex align-middle space-x-2">
      <Select
        onValueChange={async (pipelineId) => {
          ...
          const res = await fetch(
            `/api/projects/${projectId}/pipelines/${pipelineId}/versions`
          );
      ...
      >
        <SelectTrigger className="font-medium h-7">
          <SelectValue
            placeholder={defaultPipelineNameState ?? 'Select pipeline'}
      ...
      >
        <SelectTrigger
          disabled={pipelineVersions === null || pipelineVersions.length === 0}
          className="w-32 h-7 font-medium"
        >
          <SelectValue
            placeholder={defaultPipelineVersionNameState ?? 'Select version'}
