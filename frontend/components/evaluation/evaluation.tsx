    ...
    <div className="h-full flex flex-col relative">
      ...
      <div className="flex-none flex space-x-2 h-12 px-4 items-center border-b justify-start">
        <div>
          ...
          >
            <SelectTrigger disabled={evaluations.length <= 1} className="flex flex-none font-medium max-w-60 text-secondary-foreground h-7">
              <SelectValue placeholder="Select compared evaluation" />
            </SelectTrigger>
            <SelectContent>
              {evaluations.filter((item) => item.id != evaluation.id).map((item) => (
