...
export default function ScoreCard({scoreName}: ScoreCardProps) {
  ...
  const { data, isLoading, error } = useSWR(`/api/projects/${projectId}/evaluation-score-stats?evaluationId=${evaluationId}&scoreName=${scoreName}`, swrFetcher);
  const { data: comparedData, isLoading: isComparedLoading, error: isComparedError } = useSWR(
            ...
            <div className="flex flex-row">
              {!isComparedLoading && comparedData && !isComparedError && comparedData.averageValue != null && (
                <div className="flex flex-row items-center" >
                  <div className="text-5xl font-bold mr-2">{comparedData.averageValue?.toFixed(2)}</div>
                  <ArrowRight className="text-5xl font-bold mr-2" size={24} />
                </div>
              )}
              <div className="text-5xl font-bold">{data.averageValue?.toFixed(2)}</div>
            </div>
            {!isComparedLoading && comparedData && !isComparedError && comparedData.averageValue != null && (
