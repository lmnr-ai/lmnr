...
export default function Toolbar({ editable }: { editable: boolean }) {
  ...
  const nodeTypes = [
    ...
    { name: 'Code', type: NodeType.CODE, group: 'Code' },
    {
  ...
  const uniqueGroups = Array.from(
    new Set(nodeTypes.map((nodeType) => nodeType.group))
  );
      ...
      <Accordion type="multiple" defaultValue={uniqueGroups}>
        {uniqueGroups.map((groupName, groupIndex) => (
          <AccordionItem className="px-2" key={groupIndex} value={groupName}>
            <AccordionTrigger className="py-2 text-secondary-foreground text-sm hover:no-underline">
              {groupName}
            </AccordionTrigger>
                      ...
                      {NODE_PREVIEWS[nodeType.type] && (
                        <TooltipContent side="left" sideOffset={30}>
                          <NodePreviewComponent
                            name={nodeType.name}
                            description={
                              NODE_PREVIEWS[nodeType.type]!.description
                            }
