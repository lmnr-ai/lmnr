...
export default function PipelineEnv({ projectId }: PipelineEnvProps) {
  ...
  const [envVars, setEnvVars] = useState<{ [key: string]: string }>(
    getDefaultEnvVars(graph, projectId)
  );
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <Button variant="outline" className="h-7" ref={triggerRef}>
      ...
      <PopoverContent
        side="bottom"
        className="max-h-96 overflow-y-auto ml-48 mt-4 w-96"
      >
        <div className="flex flex-col items-start p-1">
          <h1 className={'text-lg'}>Environment variables</h1>
          <p
            className={cn(
              'text-sm text-muted-foreground mt-2 whitespace-normal',
              isAlerting ? 'text-red-400' : ''
            )}
          >
            ...
            <div key={key} className="mt-4 w-full">
              ...
              {ENV_VAR_TO_ISSUER_URL[key] && (
                <div className="mt-1 text-sm text-muted-foreground">
                  ...
                  <a
                    target="_blank"
                    href={ENV_VAR_TO_ISSUER_URL[key]}
