...
export default function StreamTrace({
  streamMessages,
  runTrace,
  onNodeRun
}: StreaTraceProps) {
  ...
  useEffect(() => {
    if (selectedMessage) {
      setSelectedMessage(
        streamMessages.find((m) => m.id === selectedMessage.id) || null
      );
    } else if (streamMessages.some((node) => node.reachedBreakpoint)) {
      setSelectedMessage(
        streamMessages.find((node) => node.reachedBreakpoint) || null
      );
    }
  }, [streamMessages]);
  ...
  return (
    <div className="flex h-full w-full border border-l-0">
                ...
                <div className="pl-4 cursor-pointer">
                  {streamMessage.message && (
                    <TraceCard
                      ...
                      selected={streamMessage.id === selectedMessage?.id}
                      breakpoint={selectedMessage?.reachedBreakpoint}
                    />
                  )}
                  {!streamMessage.message && (
                    <StreamTraceCard
      ...
      <div className="flex-grow flex flex-col border-l">
        <Tabs className="flex flex-col flex-grow" defaultValue="output">
          <div className="border-b flex-none">
            <div className="flex flex-none h-12 items-center px-4 space-x-2">
              {selectedMessage && (
                <>
                  <div className="p-1 px-2 text-xs text-secondary-foreground rounded bg-secondary">
                    {selectedMessage.nodeType}
                  </div>
          ...
          <div className="flex-grow">
            <TabsContent value="output" className="h-full w-full mt-0">
            ...
            <TabsContent value="inputs" className="h-full w-full mt-0">
              <div className="flex h-full w-full">
                  ...
                  <div className="flex max-h-0">
                    <div className="p-4 flex w-full h-full flex-col space-y-4">
                      ...
                      {selectedMessage?.message?.inputMessageIds &&
                        selectedMessage.message.inputMessageIds.map(
                          (id, index) => {
                            const node = streamMessages.find(
                              (node) => node.message?.id === id
                            );
...
function StreamTraceCard({
  node,
  selected,
  showSpinner = true
}: StreamTraceCardProps) {
  return (
    <div className="mt-4 border-none relative">
      {selected && (
        <div className="absolute left-[-32px] right-[-16px] top-[-8px] bottom-[-8px] bg-blue-400/10 z-0 border-l-2 border-blue-400"></div>
      )}
...
export function TraceCard({
  ...
  return (
    <div
      ...
      onMouseEnter={() => {
        onTraceHover?.(message.nodeId);
      }}
      onMouseLeave={() => {
        onTraceHover?.(undefined);
      }}
    >
      {selected && (
        <div
          className={cn(
            'absolute left-[-32px] right-[-16px] top-[-8px] bottom-[-8px] z-0 border-l-2',
            breakpoint
              ? 'bg-yellow-200/10 border-yellow-500'
      ...
      <div className="text-md flex items-center w-full">
        <div className="flex w-full items-center space-x-2">
          ...
          <div className="text-ellipsis overflow-hidden whitespace-nowrap text-sm max-w-full">
            {message.nodeName}
          </div>
          <Label className="text-secondary-foreground">
            {getDurationString(message.startTime, message.endTime)}
          </Label>
        </div>
      </div>
    </div>
