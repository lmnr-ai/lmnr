...
export default function Pipeline({ pipeline, isSupabaseEnabled }: PipelineProps) {
  ...
  const [runGraphState, setRunGraphState] = useState<RunGraphState>(
    RunGraphState.Idle
  );
  ...
  const [isUpdatingByAnotherClient, setIsUpdatingByAnotherClient] =
  ...
  const updateSelectedPipelineVersion = (versionId: string) => {
    setFocusedNodeId(null);
    fetch(
      `/api/projects/${projectId}/pipelines/${pipeline.id}/versions/${versionId}`,
      {
        method: 'GET',
        cache: 'no-store'
      }
  ...
  useEffect(() => {
    ...
    const newChannel = supabase.channel(
      'pipeline_versions_' + selectedPipelineVersion.id!
    );
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        const presenceUsers = Object.values(newState)
          .map((u: any) => ({
            ...
            imageUrl: u[0].imageUrl
          }))
          .filter((user) => user.id != currentPresenceUser.id)
  ...
  const handleInitialSync = (e: any) => {
    ...
    if (
      ...
      payload.pipelineVersionId !== selectedPipelineVersion?.id
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (
      mode === PipelineExecutionMode.Node &&
      prevMode === PipelineExecutionMode.Pipeline
    ) {
      convertStoredInputToUnseen(selectedPipelineVersion.id!, null);
    } else if (
      mode === PipelineExecutionMode.Pipeline &&
      prevMode === PipelineExecutionMode.Node
    ) {
      convertStoredInputToUnseen(
        selectedPipelineVersion.id!,
        focusedNodeId as any
      );
    } else if (
      mode === PipelineExecutionMode.Node &&
      ...
      convertStoredInputToUnseen(
        selectedPipelineVersion.id!,
        prevFocusedNodeId as any
      );
    }

    let inputNodes: InputNode[] = [];
    if (mode === PipelineExecutionMode.Node && focusedNodeId) {
      inputNodes = Array.from(getRunGraph().nodes.values()).filter(
    ...
    if (mode === PipelineExecutionMode.Node && focusedNodeId) {
      localPipelineInputs = getStoredInputs(
        selectedPipelineVersion.id!,
        focusedNodeId
      );
    } else {
    ...
    <div className="pipeline flex flex-col h-full w-full">
      <Header path={'pipelines/' + pipeline.name}>
        <PipelineHeader
          ...
          onLeftPanelOpenChange={(open) => setLeftPanelOpen(open)}
          onRightPanelOpenChange={(open) => setRightPanelOpen(open)}
      ...
      <div className="flex flex-grow h-full">
        <div
          className={cn(
            ...
            leftPanelOpen ? 'w-[22vw] border-r ' : 'w-0'
          )}
        >
          <div className="p-4">
            {selectedPipelineVersion?.pipelineType === 'COMMIT' && (
              <div>
                <Label className="text-purple-400">Commit version</Label>
              </div>
            )}
              ...
              <Button
                onClick={() => {
                  if (
                    ...
                    flowPanelRef.current.getSize() > 90
                  ) {
          ...
          {selectedPipelineVersion && <PipelineTrace />}
        </div>
        <div className="content-view flex flex-grow flex-col h-full">
          <ResizablePanelGroup direction="vertical">
                    ...
                    >
                      <div className="bg-background w-full">
                        <div className="flex flex-col relative w-full h-full">
                          <div className="h-12 pl-3 items-center flex flex-none border-b justify-between space-x-4">
                            ...
                            <div className="flex">
                              <div className="flex items-center text-secondary-foreground space-x-2 pr-4">
                                ...
                                <Switch
                                  checked={breakpointNodeIds.includes(
                          ...
                          <PipelineSheet
                            editable={
                              selectedPipelineVersion?.pipelineType ===
                              'WORKSHOP'
                            }
                          />
                        </div>
            ...
            <ResizablePanel
              defaultSize={bottomPanelMinSize}
