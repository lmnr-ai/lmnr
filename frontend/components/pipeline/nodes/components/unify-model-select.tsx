...
export default function UnifyModelSelect({
  ...
  const [selectedModelName, setSelectedModelName] = useState(
    selectableModelNames.includes(savedModelName)
      ? savedModelName
      : TYPE_MANUALLY
  );
  const [typedModelName, setTypedModelName] = useState(
    selectableModelNames.includes(savedModelName) ? '' : savedModelName
  );
    ...
    <div className="flex flex-col space-y-2">
      ...
      >
        ...
        <SelectContent>
          ...
          {selectableProviders.map((provider, i) => (
            <SelectItem
              className={dynamicProviders.includes(provider) ? 'font-bold' : ''}
              key={`provider-select-${i}`}
              value={provider}
            >
              {provider}
            </SelectItem>
      ...
      <div className="flex flex-col space-y-2">
        {metrics.map((metric, i) => (
          <div
            key={`metric-${i}`}
            className="flex h-10 rounded bg-secondary p-2 border group items-center"
          >
            <Input
              type="number"
              value={metric.float}
      ...
      <Button
        ...
        onClick={() => {
          setMetrics([
            ...metrics,
            { float: 0.5, metric: 'input-cost' } as UnifyThreshold
          ]);
          onModelChange({
            metrics: metrics
          } as UnifyNode);
