...
const GenericNodeComponent = ({
  ...
  return (
    <>
      <div
        className={cn(
          'transition-all absolute flex left-[2px] space-x-2 top-[-32px] w-72 items-center align-middle justify-center',
          isSelected ? '' : 'hidden'
        )}
      >
        <Button
            ...
            <div className={cn('flex', data.collapsed ? 'w-60' : '')}>
              <div className="font-medium w-full">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const obsoleteKeys = Array.from(
                            currentDataKeys
                          ).filter((key) => !newDataKeys.has(key));

                          // add missing keys with default values amd remove obsolete keys
                          const newData: any = {
                            ...data,
                            ...Object.fromEntries(
                              missingKeys.map((key) => [key, defaultData[key]])
                            )
                          };
                          ...
                          for (const handle of data.inputs
                            .concat(data.dynamicInputs ?? [])
                            .concat(data.outputs)) {
                            dropEdgeForHandle(handle.id);
                          }
                ...
                <div className="flex">
                  <div
                    className={
                      'z-10 flex flex-1 min-w-[50%] flex-col justify-center space-y p-2 pt-0 border-t' +
                      (inputs.length > 0 ? '' : ' hidden')
                    }
                  >
                    {inputs.map((input, i) => (
                      <div key={i} className="">
                        <Label className="text-xs text-gray-500">
                          {input.type}{' '}
                          {input.secondType ? ' | ' + input.secondType : ''}
                        </Label>
                        <div className="flex items-center relative">
                          ...
                          />
                          <div className="text-xs w-full text-left truncate">
                            {input.name}
                          </div>
                        </div>
                      </div>
                  ...
                  <div
                    className={
                      'flex flex-1 flex-col justify-center min-w-[50%] space-y border-t p-2 pt-0' +
                      (data.outputs.length > 0 ? '' : ' hidden')
                    }
                  >
