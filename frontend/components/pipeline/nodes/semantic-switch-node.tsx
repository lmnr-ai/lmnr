...
export default function SemanticSwitchNodeComponent({
  data
}: {
  data: SemanticSwitchNode;
}) {
      ...
      <div>
        ...
        <div className="flex flex-col space-y-4">
          {data.routes?.map((route, index) => (
            <div
              key={index}
              className="flex flex-col border rounded p-2 space-y-2 group"
            >
                ...
                onChange={(e) => {
                  updateNodeData(id, {
                    routes: data.routes.map((r, i) =>
                      i === index ? { ...r, name: e.currentTarget.value } : r
                    ),
                    outputs: data.outputs.map((output, i) =>
                      i === index
                        ? { ...output, name: e.currentTarget.value }
                        : output
                    )
                  } as SemanticSwitchNode);
                }}
              ...
              <Label className="text-gray-500">
                You should provide at least 2 examples
              </Label>
              <div className="flex flex-col space-y-2 justify-start">
                {route.examples?.map((example, i) => (
                  <div key={i} className="flex items-center">
                    ...
                    >
                      <X size={16} className="ml-1 text-gray-500" />
                    </button>
                  </div>
                ))}
                <Button
