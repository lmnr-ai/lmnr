...
import { FETCH_SIZE } from "@/components/playgrounds/constants";
...
export const playgroundsColumnLabels = columns.map((col) => ({
  id: col.id!,
...
export interface PlaygroundsTableContentsProps {
  filter: string[];
  search: string | null;
  isViewLoading: boolean;
}

export const PlaygroundsTableContents = memo(function PlaygroundsTableContents({
  children,
  filter,
  search,
  isViewLoading,
}: PropsWithChildren<PlaygroundsTableContentsProps>) {
  ...
  return (
    <InfiniteDataTable<PlaygroundInfo>
      className="w-full"
