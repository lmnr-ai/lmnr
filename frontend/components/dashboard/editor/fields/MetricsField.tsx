            ...
            {field.fn === "custom" && (
              <div className="grid gap-1.5 pl-1">
                ...
                <div className="h-20 flex flex-1 border rounded-md overflow-hidden">
                  <SQLEditor
                    value={field.rawExpression ?? ""}
                    onChange={(val) => update(index, { ...field, rawExpression: val })}
                    editable
                    placeholder="e.g. sum(total_cost) / count(*)"
                    schema={sqlSchema}
                    generationMode="dashboard-expression"
                    inputPlaceholder="e.g. Calculate average cost per trace"
                    projectId={projectId as string}
                  />
                </div>
                <Input
                  placeholder="Alias (e.g. cost_per_trace)"
                  value={field.alias ?? ""}
