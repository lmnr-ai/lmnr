...
function ComparisonStat({
  ...
  return (
    <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-md border border-muted text-xs font-mono">
      {icon}
      <Label className="text-xs">{leftValue}</Label>
      <ArrowRight className="size-3 text-secondary-foreground" />
      <Label className="text-xs">{rightValue}</Label>
    </div>
  );
}

function DiffSpanViewContent({ leftSpan, rightSpan }: { leftSpan: Span; rightSpan: Span }) {
  return (
    <>
...
export default function DiffSpanView({ leftTraceId, leftSpanId, rightTraceId, rightSpanId }: DiffSpanViewProps) {
  ...
  const isLoading = useMemo(() => leftLoading || rightLoading, [leftLoading, rightLoading]);
  ...
  return <DiffSpanViewContent leftSpan={leftSpan} rightSpan={rightSpan} />;
}
