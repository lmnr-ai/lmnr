...
function SelectionOverlay({ spans, containerRef, onSelectionComplete }: SelectionOverlayProps) {
  ...
  const findSpansInRect = useCallback(
    (rect: SelectionRect): Set<string> => {
      ...
      for (const span of spans) {
        ...
        if (intersects) selectedIds.add(span.span.spanId);
      }
      return selectedIds;
    },
    [spans, containerRef]
  ...
  const handleMouseDown = useCallback(
    (e: MouseEvent) => {
      ...
      if (!isPointInContainer(e)) return;
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (distance > DRAG_THRESHOLD) setIsDragging(true);
  ...
  const handleMouseUp = useCallback(() => {
    if (isDragging && selectionRect) {
      ...
      if (selectedIds.size > 0) onSelectionComplete(selectedIds);
    }
    setIsDragging(false);
