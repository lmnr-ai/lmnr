  ...
  const onSubmit = useCallback(
    async (data: AlertFormValues) => {
      ...
      try {
        ...
        toast({
          title: "Alert created",
          description: `"${data.name.trim()}" is now active and will send notifications.`,
        });
        onCreated();
        setOpen(false);
        resetForm();
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: e instanceof Error ? e.message : "Failed to create alert. Please try again.",
        });
      }
    },
    [projectId, integrationId, selectedSignal, selectedChannel, onCreated, resetForm, toast]
  );
