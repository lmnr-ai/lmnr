      ...
      try {
        ...
        if (!res.ok) {
          const errMessage = await res
            ...
            .then((d) => d?.error)
            .catch(() => null);
          setError(errMessage ?? "Failed to fetch trace data");
          return;
        }
  ...
  return (
    <div className="flex flex-1 flex-col min-h-0 w-full overflow-hidden">
      ...
      {isFetching ? (
        ...
        <div className="flex flex-1 items-center justify-center px-4 text-center text-xs text-destructive">{error}</div>
      ) : (
