...
const AdvancedFilterInput = ({
  ...
  const handleValueSuggestionSelect = useCallback((tagId: string, value: string) => {
    // Update tag value and submit
    setFilterTags((prev) =>
      prev.map((tag) => (tag.id === tagId ? { ...tag, value } : tag))
    );
    setActiveTagId(null);
    ...
    // Submit with the new value
    const updatedTags = filterTags.map((tag) =>
      tag.id === tagId ? { ...tag, value } : tag
    );
    submit(updatedTags, inputValue);
  }, [filterTags, inputValue, submit]);
  ...
  return (
    ...
    >
      <FilterInputArea
        ...
        onValueSuggestionSelect={handleValueSuggestionSelect}
      />
