...
import { defaultTracesColumnOrder, filters, STATIC_COLUMNS } from "@/components/traces/traces-table/columns";
import TracesColumnsMenu from "@/components/traces/traces-table/traces-columns-menu";
import {
  buildTracesColumnDefs,
  buildTracesCustomColumnDef,
  buildTracesFetchParams,
...
const LOCKED_COLUMNS = ["status"];

export default function TracesTable() {
  return (
    <DataTableStateProvider<TraceRow>
      storageKey="traces-table"
      defaultColumnOrder={defaultTracesColumnOrder}
      initialColumnDefs={STATIC_COLUMNS}
      lockedColumns={LOCKED_COLUMNS}
      buildCustomColumnDef={buildTracesCustomColumnDef}
    >
      <TracesTableContent />
    </DataTableStateProvider>
  );
...
function TracesTableContent() {
  ...
  // Read columnDefs from the datatable store
  const datatableStore = useDataTableStore<TraceRow>();
  const columnDefs = useStore(datatableStore, (s) => s.columnDefs);

  // Rebuild column defs when custom columns change (persisted state hydrated)
  ...
  // Rebuild on hydration of custom columns
  const rebuiltDefs = useMemo(() => buildTracesColumnDefs(customColumns), [customColumns]);
  // Only call setColumnDefs when the built defs actually differ
  ...
  const columnSqls = useStore(datatableStore, selectColumnSqls);
  ...
  const fetchTraces = useCallback(
    async (pageNumber: number) => {
      try {
        const urlParams = buildTracesFetchParams(columnDefs, {
          pageNumber,
          pageSize: FETCH_SIZE,
          filter,
          sortBy: sortBy ?? null,
