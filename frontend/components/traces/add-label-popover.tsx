...
export function AddLabelPopover({ span }: AddLabelPopoverProps) {
  ...
  const updateLabelClass = async (labelClass: LabelClass) => {
    const res = await fetch(
      `/api/projects/${projectId}/label-classes/${labelClass.id}`,
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  const registerLabelClass = async (labelClass: LabelClass) => {
    const res = await fetch(
      `/api/projects/${projectId}/label-classes/${labelClass.id}/registered-paths`,
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  const unregisterLabelClass = async (
    registeredLabelClass: RegisteredLabelClassForPath
  ) => {
    const res = await fetch(
                          ...
                          <TableCell className="px-0">
                            <span className="text-sm">
                              {labelClass.evaluatorRunnableGraph && (
                                <Switch
                                  checked={registeredLabelClasses?.some(
                                    (l) => l.labelClassId === labelClass.id
                                  )}
                                  onCheckedChange={(value) => {
                                    if (value) {
                                      ...
                                      unregisterLabelClass(
                                        registeredLabelClasses?.find(
                                          (l) =>
                                            l.labelClassId === labelClass.id
                                        )!
                                      );
                                    }
                                  }}
                                  ...
                                  <EvaluatorEditorDialog
                                    ...
                                    onEvaluatorAdded={(
                                      ...
                                      updateLabelClass({
                                        ...labelClass,
                                        evaluatorRunnableGraph:
                                          evaluatorRunnableGraph.toObject()
                                      });
                                    }}
                                  >
