...
const sendExportRequest = async (
  ...
  await fetch(
    `/api/projects/${projectId}/traces/endpoint/${endpointId}/export`,
    {
      method: 'POST',
      body: body
    }
  ...
  return useAll || runIds.length > 0 ? (
    ...
    >
      ...
      <DialogContent>
        ...
        <div className="text-secondary-foreground flex flex-col space-y-2">
          <Label>
            {' '}
            Exporting {useAll ? totalNumberOfTraces : runIds.length} selected
            traces.
          </Label>
        </div>
        <DatasetSelect
          onDatasetChange={(dataset) => setSelectedDatasetId(dataset.id)}
        />
        <DialogFooter>
          <Button
            variant="default"
            ...
            onClick={async () => {
              ...
              const filters = useAll
                ? undefined
                : queryParamFilters
                  ? (getFilterFromUrlParams(queryParamFilters) ?? [])
                  : [];
              try {
    ...
    <TooltipProvider delayDuration={200}>
      <Tooltip>
        ...
        <TooltipContent side="bottom">Select traces to export</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
