...
export function ProviderButton({ icon, label, onClick, isLoading, isDisabled }: ProviderButtonProps) {
  return (
    <Button
      ...
      className="text-[16px] py-6 px-4 pr-6 w-full bg-surface-800 hover:bg-surface-600"
    >
      <div className="h-5 w-5 text-foreground-100">{isLoading ? <IconSpinner className="animate-spin" /> : icon}</div>
