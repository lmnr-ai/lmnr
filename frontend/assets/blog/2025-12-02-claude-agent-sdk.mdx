TL;DR: We’ve released instrumentation packages for the claude-agent-sdk in both Python and TypeScript for developers to monitor and debug their custom Claude agents. Both SDKs use a tiny, transparent Rust proxy to capture every prompt, tool call, and latency metric from the Claude Code process locally and efficiently.
...
We considered spinning up a side endpoint on the proxy to say "Request ABC belongs to Trace 123," but this created too much overhead:
...
Zero-Friction Setup: The solution works out of the box with just installation and initialization—no configuration of environment variables or base URLs is required.
