...
type LLMProvider = "openai" | "gemini" | "bedrock" | "foundry";
...
const DEFAULT_MODELS: Record<LLMProvider, Record<ModelTier, string>> = {
  ...
  // Foundry model ids are deployment names — these only hold when deployments
  // are named after the model; otherwise set LLM_MODEL_<TIER>.
  foundry: {
...
export function getLanguageModel(tier: ModelTier = "large"): LanguageModel {
  ...
  if (!provider) {
    throw new Error(
      "No AI provider configured. Set LLM_PROVIDER to openai, gemini, foundry, or bedrock. " +
