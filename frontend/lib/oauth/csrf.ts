 ...
 *
 * We compare canonical `URL.origin` (scheme + host + port) so trailing-slash
 * and path variants on `NEXTAUTH_URL` do not cause false rejections.
 *
...
export function requireSameOrigin(req: NextRequest): Response | null {
  ...
  try {
    ...
    expectedOrigin = new URL(getIssuer()).origin;
  } catch {
