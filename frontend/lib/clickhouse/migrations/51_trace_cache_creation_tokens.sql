-- LAM-1807: Denormalize per-trace cache-creation (Anthropic prompt-caching write)
-- token sums onto the trace row, alongside the existing cache_read / reasoning
-- columns (migration 49). Aggregated at ingestion time in app-server
-- (TraceAggregation::from_spans, sourced from InputTokens.cache_write_tokens) and
-- summed across batches by the ReplacingMergeTree upsert.
...
-- Recreate raw_traces_v0, mirroring migration 49 and exposing the new column.
