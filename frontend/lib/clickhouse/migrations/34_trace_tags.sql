-- 1. Rename the existing `tags` table to `span_tags`
RENAME TABLE IF EXISTS default.tags TO default.span_tags;

-- 2. Create the `trace_tags` table (same schema as span_tags but with trace_id instead of span_id)
CREATE TABLE IF NOT EXISTS default.trace_tags
(
    `project_id` UUID,
    `created_at` DateTime64(9, 'UTC'),
    `id` UUID,
    `name` String,
    `source` UInt8,
    `trace_id` UUID
)
ENGINE = MergeTree
ORDER BY (project_id, name, created_at)
SETTINGS index_granularity = 8192;

ALTER TABLE default.trace_tags ADD INDEX IF NOT EXISTS trace_tags_created_at_minmax_idx created_at TYPE minmax;
ALTER TABLE default.trace_tags ADD INDEX IF NOT EXISTS trace_tags_trace_id_bf_idx trace_id TYPE bloom_filter;

-- 3. Rename `tags` column on traces_replacing to `span_tags`, add `trace_tags` column
ALTER TABLE default.traces_replacing RENAME COLUMN IF EXISTS `tags` TO `span_tags`;
ALTER TABLE default.traces_replacing ADD COLUMN IF NOT EXISTS `trace_tags` Array(String) DEFAULT [];

-- 4. Recreate views to reflect new column names
DROP VIEW IF EXISTS default.raw_traces_v0;
CREATE VIEW IF NOT EXISTS default.raw_traces_v0 SQL SECURITY INVOKER AS
SELECT
    start_time,
    end_time,
    input_tokens,
    output_tokens,
    total_tokens,
    input_cost,
    output_cost,
    total_cost,
    duration,
    metadata,
    session_id,
    user_id,
    CASE
        WHEN status = 'error' THEN 'error'
        ELSE 'success'
    END AS status,
    top_span_id,
    top_span_name,
    CASE
        WHEN top_span_type = 0 THEN 'DEFAULT'
        WHEN top_span_type = 1 THEN 'LLM'
        WHEN top_span_type = 3 THEN 'EXECUTOR'
        WHEN top_span_type = 4 THEN 'EVALUATOR'
        WHEN top_span_type = 5 THEN 'EVALUATION'
        WHEN top_span_type = 6 THEN 'TOOL'
        WHEN top_span_type = 7 THEN 'HUMAN_EVALUATOR'
        WHEN top_span_type = 8 THEN 'CACHED'
        ELSE 'UNKNOWN'
    END AS top_span_type,
    CASE
      WHEN trace_type = 3 THEN 'PLAYGROUND'
      WHEN trace_type = 1 THEN 'EVALUATION'
      WHEN trace_type = 0 THEN 'DEFAULT'
      ELSE 'DEFAULT'
    END AS trace_type,
    arrayDistinct(span_tags) AS span_tags,
    arrayDistinct(trace_tags) AS trace_tags,
    has_browser_session,
    arrayDistinct(span_names) span_names,
    root_span_input,
    root_span_output,
    id,
    project_id
FROM default.traces_replacing FINAL
WHERE project_id={project_id:UUID};

DROP VIEW IF EXISTS default.traces_v0;
CREATE VIEW IF NOT EXISTS default.traces_v0 SQL SECURITY INVOKER AS
SELECT
    t.start_time AS start_time,
    t.end_time AS end_time,
    t.input_tokens AS input_tokens,
    t.output_tokens AS output_tokens,
    t.total_tokens AS total_tokens,
    t.input_cost AS input_cost,
    t.output_cost AS output_cost,
    t.total_cost AS total_cost,
    t.duration AS duration,
    t.metadata AS metadata,
    t.session_id AS session_id,
    t.user_id AS user_id,
    t.status AS status,
    t.top_span_id AS top_span_id,
    t.top_span_name AS top_span_name,
    t.top_span_type AS top_span_type,
    t.trace_type AS trace_type,
    t.span_tags AS span_tags,
    t.trace_tags AS trace_tags,
    t.has_browser_session AS has_browser_session,
    t.id AS id,
    t.span_names AS span_names,
    t.root_span_input AS root_span_input,
    t.root_span_output AS root_span_output
FROM
    default.raw_traces_v0(project_id={project_id:UUID}) t
WHERE t.project_id={project_id:UUID};

-- 5. Recreate the tags_v0 view to point at span_tags table
DROP VIEW IF EXISTS default.tags_v0;
CREATE VIEW IF NOT EXISTS default.tags_v0 SQL SECURITY INVOKER AS
SELECT
    id,
    span_id,
    name,
    created_at,
    CASE
        WHEN source = 0 THEN 'HUMAN'
        WHEN source = 2 THEN 'CODE'
        ELSE 'UNKNOWN'
    END source
FROM span_tags
WHERE project_id={project_id:UUID};
