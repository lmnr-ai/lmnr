...
import { SEVERITY_LEVELS } from "@/lib/actions/alerts/types";
...
export async function createSignal(input: z.infer<typeof CreateSignalSchema>) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    // Auto-create an alert for the new signal with severity = Critical
    const [alert] = await tx
      ...
      .values({
        ...
        type: "SIGNAL_EVENT",
        sourceId: signal.id,
        metadata: { severity: SEVERITY_LEVELS.CRITICAL },
      })
