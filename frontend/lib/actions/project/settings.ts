 ...
 * defaults. The pre-`piiMode` `removePii` toggle (left in the JSONB by
 * migration 0111 for pods still on the previous app-server binary) is
 * translated (`true` → `redact`) when `piiMode` is absent; the Rust reader
