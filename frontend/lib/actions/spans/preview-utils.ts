...
export const classifyPayload = (raw: unknown): PayloadClassification => {
  ...
  // Deep-parse all nested JSON strings so fingerprinting and LLM see the true structure
...
/**
 * Ensure section blocks in mustache templates produce newline-separated output.
 * Appends a newline before each closing tag if one isn't already there.
 */
const ensureSectionNewlines = (template: string): string => template.replace(/([^\n])({{\/[^}]+}})/g, "$1\n$2");
...
const addStringifyToObjects = (value: unknown): unknown => {
  ...
  if (typeof value === "object") {
    ...
    for (const [k, v] of Object.entries(value)) {
      result[k] = addStringifyToObjects(v);
    }
