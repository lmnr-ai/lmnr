...
export const notificationTriggers = pgTable(
  ...
  (table) => [
    foreignKey({
      ...
      name: "notification_triggers_notification_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.projectId],
      foreignColumns: [projects.id],
      name: "notification_triggers_project_id_fkey",
    }).onDelete("cascade"),
  ]
);
...
export const slackChannelToEvents = pgTable(
  ...
  (table) => [
    foreignKey({
      columns: [table.notificationId],
      foreignColumns: [notifications.id],
      name: "slack_channel_to_events_notification_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
...
export const semanticEventTriggerSpans = pgTable(
  ...
  (table) => [
    ...
    unique("semantic_event_trigger_spans_project_event_definition_span_key").on(
      table.projectId,
      table.spanName,
...
export const semanticEventDefinitions = pgTable(
