...
export const workspaceUsage = pgTable(
  "workspace_usage",
  {
    ...
    eventCount: bigint("event_count", { mode: "number" })
      ...
      .notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    eventCountSinceReset: bigint("event_count_since_reset", { mode: "number" })
      .default(sql`'0'`)
      .notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    prevEventCount: bigint("prev_event_count", { mode: "number" })
      .default(sql`'0'`)
      .notNull(),
...
export const subscriptionTiers = pgTable("subscription_tiers", {
  ...
  membersPerWorkspace: bigint("members_per_workspace", { mode: "number" })
    ...
    .notNull(),
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  numWorkspaces: bigint("num_workspaces", { mode: "number" })
    .default(sql`'-1'`)
    .notNull(),
  stripeProductId: text("stripe_product_id").default("").notNull(),
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  events: bigint({ mode: "number" })
    .default(sql`'0'`)
    .notNull(),
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  ...
  extraSpanPrice: doublePrecision("extra_span_price")
    .default(sql`'0'`)
    .notNull(),
  extraEventPrice: doublePrecision("extra_event_price")
    .default(sql`'0'`)
    .notNull(),
});
