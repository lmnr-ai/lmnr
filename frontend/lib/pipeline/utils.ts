...
const getDisconnectedNodes = (graph: Graph, edges: Edge[]): GenericNode[] => {
  ...
  return Array.from(graph.nodes.values()).filter(
    (node) => !connectedNodes.has(node.id) && node.type != NodeType.CONDITION
  );
};

const getDisconnectedInputHandles = (
  node: GenericNode
): GenericNodeHandle[] => {
  const allNodeInputs = node.inputs.concat(node.dynamicInputs || []);
  const disconnectInputHandles = allNodeInputs.filter(
    (input) => !Object.keys(node.inputsMappings ?? {}).includes(input.id)
  );
  return disconnectInputHandles;
};

const getNodesWithDisconnectedOutputHandles = (
  graph: Graph,
  edges: Edge[]
): GenericNode[] => {
  ...
  return Array.from(graph.nodes.values())
    ...
    .filter(
      (node) =>
        !node.outputs.every((output) => connectedOutputHandles.has(output.id))
    );
};
...
const markCyclicInputHandles = (graph: Graph, edges: Edge[]) => {
  ...
  const traverseFromInputHandle = (
    currentNode: GenericNode,
    startNode: GenericNode,
    startInputHandlesName: string,
    ...
    for (const nextNodesAndHandleId of nextNodesAndHandleIds) {
      ...
      const nextHandle = nextNode.inputs
        .concat(nextNode.dynamicInputs ?? [])
        .find((input) => input.id === nextNodesAndHandleId.handleId);
      if (
        nextNode.id === startNode.id &&
        nextHandle?.name === startInputHandlesName
      ) {
        nextNode.inputs
          .concat(nextNode.dynamicInputs || [])
      ...
      traverseFromInputHandle(
        nextNode,
        startNode,
        startInputHandlesName,
        visitedNodeIds
      );
    }
  };
  ...
  for (const node of graph.nodes.values()) {
    const uniqueInputHandleName = new Set(
      node.inputs.concat(node.dynamicInputs ?? []).map((input) => input.name)
    );
...
export const validateGraph = (graph: Graph, edges: Edge[]): string => {
  ...
  if (
    !nodes.some((node) => [NodeType.OUTPUT, NodeType.ERROR].includes(node.type))
  ) {
  ...
  const nodesWithDisconnectedOutputHandles =
    getNodesWithDisconnectedOutputHandles(graph, edges);
  ...
  const disconnectedInputHandles: Record<string, GenericNodeHandle[]> =
    Object.fromEntries(
  ...
  if (Object.keys(disconnectedInputHandles).length > 0) {
    return `Nodes\n${Object.entries(disconnectedInputHandles)
      .map(
        ([nodeId, handles]) =>
  ...
  if (
    nodes.some(
      (node) =>
        node.type === NodeType.SUBPIPELINE &&
        !(node as SubpipelineNode).pipelineVersionId
    )
  ) {
  ...
  for (const node of nodes) {
    if (node.type === NodeType.MAP) {
      ...
      const inputNodes = Object.values(
        (node as MapNode).runnableGraph.nodes
      ).filter((node) => node.type === NodeType.INPUT);
      if (
        inputNodes.length !== 1 ||
        inputNodes[0].outputs[0].type !== NodeHandleType.STRING
      ) {
