...
export function getRequiredEnvVars(nodes: GenericNode[]): Set<string> {
  ...
  for (const node of nodes) {
    if ([NodeType.LLM].includes(node.type)) {
      ...
      MODEL_PROVIDER_TO_API_KEYS[modelProvider].forEach((envVar) =>
        vars.add(envVar)
      );
      ...
      const envVars = getRequiredEnvVars(
        Array.from(Object.values((node as SubpipelineNode).runnableGraph.nodes))
      );
      ...
      const envVars = getRequiredEnvVars(
        Array.from(Object.values((node as MapNode).runnableGraph.nodes))
      );
      for (const envVar of envVars) {
        vars.add(envVar);
      }
...
export const ENV_VAR_TO_ISSUER_URL: Record<string, string> = {
  ...
  [EnvVars.GOOGLE_SEARCH_ENGINE_ID]:
