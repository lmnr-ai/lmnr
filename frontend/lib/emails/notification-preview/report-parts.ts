...
export function clusterGlyph(color: string): string {
  return `<span aria-hidden="true" style="display:inline-block;color:${color};font-family:Arial,sans-serif;font-size:20px;font-weight:400;line-height:20px;vertical-align:middle;">◇</span>`;
}

export function viewButton(t: NotificationEmailTheme, href: string, inClusterRow = false): string {
  const className = inClusterRow ? "email-view-button email-cluster-view-button" : "email-view-button";
  const background = inClusterRow ? "rgba(0,0,0,0.08)" : t.reportChartFill;
  return `<a class="${className}" href="${href}" style="display:inline-block;background:${background};border-radius:999px;color:${t.text};font-size:${t.metaSize}px;line-height:16px;padding:4px 10px;text-decoration:none;white-space:nowrap;">View&nbsp;›</a>`;
}
...
/** One pill row. The proportion bar is a hard-stop linear-gradient rather than
...
export function clusterRow(t: NotificationEmailTheme, row: ClusterRow, maxCount: number): string {
  ...
  return `<table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="margin-bottom:4px;${bar}border-radius:${t.reportRowRadius}px;">
  <tr>
    <td style="padding:6px 12px 6px 10px;font-size:${t.bodySize}px;color:${t.text};" align="left" valign="middle">
      <table cellpadding="0" cellspacing="0" border="0" role="presentation"><tr>
        <td width="24" valign="middle" style="line-height:0;">${clusterGlyph(accent)}</td>
    ...
    <td style="padding:3px 4px 3px 8px;" align="right" width="50">
      ${viewButton(t, row.href, true)}
    </td>
  </tr>
</table>`;
}
...
export function barChart(t: NotificationEmailTheme, buckets: ChartBucket[], fillOverride?: string): string {
  ...
  const fill = fillOverride ?? t.reportChartFill;
  const cells = buckets
    .map((bucket) => {
