...
test("surfaceClasses maps a level (1..8) to bg + shadow + the raise var", () => {
  assert.equal(surfaceClasses(1), "bg-surface-100 shadow-elevation-100 [--surface-raise:var(--color-surface-300)]");
  assert.equal(surfaceClasses(3), "bg-surface-300 shadow-elevation-300 [--surface-raise:var(--color-surface-500)]");
...
test("surfaceClasses rounds fractional levels so it never indexes out of the table", () => {
  assert.equal(surfaceClasses(2.4), "bg-surface-200 shadow-elevation-200 [--surface-raise:var(--color-surface-400)]");
  assert.equal(surfaceClasses(2.6), "bg-surface-300 shadow-elevation-300 [--surface-raise:var(--color-surface-500)]");
});
