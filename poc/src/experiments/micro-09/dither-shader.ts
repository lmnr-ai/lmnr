 ...
 * The repository copy of the license and notice is in ../micro-08/vendor/paper/.
 */
import {BAYER_8} from '../micro-08/dither';
...
void main() {
  ...
  vec2 uv = (screen - u_imageRect.xy) / u_imageRect.zw;
  ...
  vec4 sampled = texture(u_image, (pixel - u_imageRect.xy) / u_imageRect.zw);
  vec3 source = sampled.a > .00001 ? sampled.rgb / sampled.a : original.rgb / original.a;
  float lum = clamp((dot(vec3(.2126, .7152, .0722), source) - .5) * u_contrast + .5, 0., 1.);
  float steps = max(floor(u_colorLevels) - 1., 1.);
  float brightness = clamp(lum + (getBayerValue(cell) - .5) / steps, 0., 1.);
  vec3 filtered = vec3(floor(brightness * steps + .5) / steps) * original.a;
