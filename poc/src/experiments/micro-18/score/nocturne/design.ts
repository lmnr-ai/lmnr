...
export const designInKey = (key: number) => (mix: Mix, cues: ScoreCues) => {
  ultimate2(mix, cues, key);
  cost(mix, cues, key);
  flow(mix, cues, key);
  issues(mix, cues, key);
  conclusion(mix, cues, key);
};
...
function alert(mix: Mix, time: number, key: number) {
  beep(mix, time, key + 88, .5, {...TECH, pan: -.08}, {length: .09, wave: 'triangle'});
  beep(mix, time + .13, key + 85, .46, {...TECH, pan: .08}, {length: .16, wave: 'triangle'});
  beep(mix, time, key + 76, .2, TECH, {length: .3});
  thock(mix, time, .3, TOUCH, .8);
}

function ultimate2(mix: Mix, cues: ScoreCues, key: number) {
  const u2 = cues.ultimate2;
  beep(mix, u2.agentEnter + .01, key + 87, .26, {...TECH, pan: 0}, {length: .05});
  beep(mix, u2.agentEnter + .09, key + 94, .18, {...TECH, pan: .1}, {length: .05});
  ...
  beep(mix, u2.failure, key + 75, .32, {...TECH, pan: .15}, {length: .9, glide: -12, attack: .004});
  ...
  u2.drawers.forEach((time, i) => {
    ...
    tick(mix, time + .24, key + (84 + i * 3), .22, {...TOUCH, pan: -.25 + i * .25}, .008);
  });
  beep(mix, u2.highlight.at, key + 91, .16, {...TECH, pan: .2}, {length: .04});
  alert(mix, u2.warning, key);
  ...
  for (let n = 0; n < 9; n++) beep(mix, u2.collapse.at + n * .06 + mix.random() * .02, key + [99, 96, 94, 91, 89, 87, 84, 82, 79][n], .12 - n * .008, {...TECH, pan: n % 2 ? .45 : -.45}, {length: .025});
  whoosh(mix, u2.cloudIn.at, u2.cloudIn.duration, AIR, {from: 180, to: 800, level: .36, peak: .75, q: .9, panFrom: .4, panTo: -.1});
}

function cost(mix: Mix, cues: ScoreCues, key: number) {
  const c = cues.cost;
  ...
  for (const leg of c.cheapLegs) {
    ...
    beep(mix, leg.at + leg.duration * .8, key + 96, .1, {...TECH, pan: .5 * direction}, {length: .03});
  }
  thock(mix, c.thinkingDrop.at + c.thinkingDrop.duration - .05, .22, TOUCH, 1.3);
  // "…fail to find crucial issues": a flat negative blip.
  beep(mix, c.missIssues, key + 64, .2, {...TECH, pan: 0}, {length: .18, wave: 'square'});
  ...
  for (let t = 0; t < descent.duration; ) {
    ...
    beep(mix, descent.at + t, key + (96 - Math.round(progress * 5) + (mix.random() > .7 ? 5 : 0)), .07 * (1 - progress * .4), {...TECH, pan: (mix.random() - .5) * .6}, {length: .018});
    t += 1 / (22 - 14 * progress ** 2);
  }
  alert(mix, c.bashWarning, key);

  whoosh(mix, c.cameraToBudget.at, c.cameraToBudget.duration, AIR, {from: 1400, to: 260, level: .26, peak: .5, panFrom: -.1, panTo: .1});
  beep(mix, c.budgetAppear, key + 87, .2, {...TECH, pan: -.1}, {length: .06});
  beep(mix, c.budgetAppear + .08, key + 94, .16, {...TECH, pan: .1}, {length: .09});
  ...
  for (let at = 0; at < depletion.duration - .1; ) {
    ...
    beep(mix, depletion.at + at, key + Math.round(91 - 22 * progress ** 1.3), .11 * (1 - progress * .5), {...TECH, pan: -.2 + .4 * progress}, {length: .03});
    at += .08 + .3 * progress ** 2;
  }
}

function flow(mix: Mix, cues: ScoreCues, key: number) {
  const f = cues.flow;
  ...
  f.numberDrops.forEach((time, n) => beep(mix, time, key + ([91, 87, 94, 84, 82, 79][n] ?? 79), n === 2 ? .26 : .14, {...TECH, pan: -.3 + n * .12}, {length: n === 2 ? .12 : .04}));
  for (let n = 0; n < 10; n++) beep(mix, f.countUp.at + f.countUp.duration * (n / 10) ** .7, key + (82 + n), .07, {...TECH, pan: .15}, {length: .018});

  whoosh(mix, f.cameraToAnalysis.at, f.cameraToAnalysis.duration, AIR, {from: 600, to: 2600, level: .24, peak: .5, panFrom: .5, panTo: -.3});
  for (let n = 0; n < 4; n++) beep(mix, f.numberSwap.at + n * f.numberSwap.duration / 4, key + [87, 84, 87, 91][n], .12, {...TECH, pan: -.1}, {length: .03});
  for (let n = 0; n < 8; n++) beep(mix, f.analysisCountUp.at + f.analysisCountUp.duration * (n / 8) ** .6, key + (87 + n), .06, TECH, {length: .016});
  ...
  beep(mix, f.moduleActivation, key + 82, .22, {...TECH, pan: 0}, {length: .14, glide: 12});
  beep(mix, f.moduleActivation + .15, key + 94, .18, TECH, {length: .08});
  for (let n = 0; n < 8; n++) beep(mix, f.engineSpinner.at + n * f.engineSpinner.duration / 8, key + (n % 2 ? 91 : 94), .06, {...TECH, pan: n % 2 ? .3 : -.3}, {length: .02});
...
function issues(mix: Mix, cues: ScoreCues, key: number) {
  const i = cues.issues;
  ...
  for (const item of i.pops) {
    ...
    beep(mix, item.at, key + midi, .09 + mix.random() * .04, {...TECH, pan: item.pan * .9}, {length: .03});
  }
  for (let n = 0; n < 4; n++) {
    const pan = n % 2 ? .5 : -.5;
  ...
  i.clusters.forEach((time, n) => beep(mix, time, key + [87, 91, 94, 99, 94, 91][n % 6], .12, {...TECH, pan: -.5 + (n % 6) * .2}, {length: .05}));
  ...
  for (const time of [i.issueBadge, i.queryBadge]) beep(mix, time, key + 87, .18, {...TECH, pan: .2}, {length: .06});
  whoosh(mix, i.messageSend - .04, .3, TOUCH, {from: 900, to: 3800, level: .18, q: 1.8, peak: .7, panFrom: -.2, panTo: .4, air: .4});
  beep(mix, i.messageSend + .2, key + 91, .16, {...TECH, pan: .3}, {length: .05});
  beep(mix, i.messageSend + .28, key + 96, .13, {...TECH, pan: .35}, {length: .07});
  whoosh(mix, i.windowUp.at, i.windowUp.duration, AIR, {from: 400, to: 1800, level: .24, peak: .5});
}

function conclusion(mix: Mix, cues: ScoreCues, key: number) {
  ...
  [[.5, 87], [.62, 94], [.74, 99]].forEach(([delay, midi], n) => beep(mix, logo + delay, key + midi, .12 - n * .02, {...TECH, pan: -.15 + n * .15}, {length: .08}));
}
