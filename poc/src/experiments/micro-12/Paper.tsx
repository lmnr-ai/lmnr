...
// Same door/paper/highlight choreography and copy as Animation 7, no elbow.
export const Paper = ({x, width, kind, progress, highlight}: {x: number; width: number; kind: string; progress: number; highlight: number}) => {
  ...
  return <foreignObject x={x} y={-60} width={width} height={260} style={{overflow: 'hidden', clipPath: `inset(${120 * (1 - progress)}px 0 0 0)`}}>
    <div className="micro12-paper" style={{width, whiteSpace: kind === 'thinking-blue' ? 'normal' : 'pre-line', transform: `translateY(${(120 - 260) * (1 - progress)}px)`}}>
      {kind !== 'thinking-blue' ? copy : PAPER_LINES.map((line, index) => {
        ...
        return <div key={index} className="micro12-paper-line" style={{lineHeight: `${PAPER_LINE_HEIGHT}px`}}>{line.text}{fragment && <span aria-hidden="true" className="micro12-highlight" style={{left: fragment.column * PAPER_ADVANCE - 2, top: -2, width: fragment.length * PAPER_ADVANCE + 4, padding: 2, clipPath: `inset(0 ${100 * (1 - amount)}% 0 0)`}}>{fragment.text}</span>}</div>;
      })}
    </div>
  </foreignObject>;
};
